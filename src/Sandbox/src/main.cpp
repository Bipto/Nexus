...
public:
  ...
  virtual void Load() override {
    /* m_Texture =
    m_GraphicsDevice->CreateTexture(Nexus::FileSystem::GetFilePathAbsolute("resources/textures/brick.jpg"),
    false); m_BatchRenderer = new
    Nexus::Graphics::BatchRenderer(m_GraphicsDevice,
    Nexus::Graphics::RenderTarget{m_GraphicsDevice->GetPrimaryWindow()->GetSwapchain()});

    rrect = Nexus::Graphics::RoundedRectangle(100, 100, 500, 500, 25, 25, 25,
    25);

    m_Canvas = std::make_unique<Nexus::UI::Canvas>(m_GraphicsDevice,
    m_GraphicsDevice->GetPrimaryWindow()->GetSwapchain());

    std::vector<Nexus::Graphics::CharacterRange> fontRange = {{0x0020, 0x00FF}};

    m_Font = new
    Nexus::Graphics::Font("resources/Fonts/JETBRAINSMONO-REGULAR.TTF", 18,
    fontRange, Nexus::Graphics::FontType::Bitmap, m_GraphicsDevice);
    ...
    lbl->SetText("Hello World!Even more text...\n\tHopefully this second line is
    longer than the first:)"); lbl->SetFontSize(m_Font->GetSize());
    ...
    btn->OnMouseClick += [&](Nexus::UI::Control *ctrl) { std::cout << "Button
    clicked\n"; };
