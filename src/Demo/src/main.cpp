...
struct DemoInfo {
  ...
  Demos::Demo *(*CreationFunction)(
      Nexus::Application *, const std::string &name,
...
public:
  ...
  virtual void Load() override {
    m_ImGuiRenderer =
        std::make_unique<Nexus::ImGuiUtils::ImGuiGraphicsRenderer>(this);
    ...
    RegisterGraphicsDemo<Demos::HelloTriangleIndexedDemo>(
    ...
    RegisterUtilsDemo<Demos::ClippingAndTriangulationDemo>(
        "Polygon clipping and triangulation");
  ...
  template <typename T> void RegisterGraphicsDemo(const std::string &name) {
    ...
    info.CreationFunction =
        [](Nexus::Application *app, const std::string &name,
           Nexus::ImGuiUtils::ImGuiGraphicsRenderer *imGuiRenderer)
        -> Demos::Demo * { return new T(name, app, imGuiRenderer); };
  ...
  void RenderDemoList(std::span<DemoInfo> demos, const std::string &menuName) {
    if (ImGui::TreeNodeEx(menuName.c_str(),
      ...
      for (const auto &pair : demos) {
        ...
        if (ImGui::TreeNodeEx(pair.Name.c_str(), flags)) {
          if (ImGui::IsItemClicked()) {
            m_CurrentDemo = std::unique_ptr<Demos::Demo>(
                pair.CreationFunction(this, pair.Name, m_ImGuiRenderer.get()));
            m_CurrentDemo->Load();
          }
...

    {
      ...
      if (m_CurrentDemo) {
        ...
        if (m_CurrentDemo) {
          ...
          std::string apiName = std::string("Running on : ") +
                                std::string(m_GraphicsDevice->GetAPIName());
...
private:
  ...
  std::unique_ptr<Nexus::ImGuiUtils::ImGuiGraphicsRenderer> m_ImGuiRenderer =
      nullptr;
};
