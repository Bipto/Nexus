  ...
  virtual void Render(Nexus::Time time) override {
    m_TransformUniforms.Transform =
        glm::rotate(glm::mat4(1.0f),
                    glm::radians((float)m_ElapsedTime.GetSeconds() * 100.0f),
    ...
    m_CameraUniforms.Projection = glm::perspectiveFov<float>(
        glm::radians(60.0f), m_Window->GetWindowSize().X,
        m_Window->GetWindowSize().Y, 0.1f, 100.0f);
  ...
  void CreatePipeline() {
    ...
    pipelineDescription.VertexModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/3d.vert.glsl",
            Nexus::Graphics::ShaderStage::Vertex);
    pipelineDescription.FragmentModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/3d.frag.glsl",
