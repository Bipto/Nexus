...
public:
  ...
  virtual void Load() override {
    ...
    m_InstanceBuffer =
        m_GraphicsDevice->CreateVertexBuffer(vertexBufferDescription, nullptr);

    std::vector<glm::mat4> mvps(m_InstanceCount);
    for (uint32_t i = 0; i < m_InstanceCount; i++) {
      mvps[i] =
          glm::translate(glm::mat4(1.0f), glm::vec3(i * 2.0f, 0.0f, 2.5f));
    }
  ...
  void CreatePipeline() {
    ...
    pipelineDescription.VertexModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/instancing.vert.glsl",
            Nexus::Graphics::ShaderStage::Vertex);
    pipelineDescription.FragmentModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/instancing.frag.glsl",
