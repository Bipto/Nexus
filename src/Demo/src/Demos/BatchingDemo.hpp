...
public:
  ...
  void GenerateShapes() {
    ...
    std::mt19937 rng(dev());
    std::uniform_real_distribution<float> random_x(0, size.X -
                                                          (m_MaxQuadSize / 2));
    std::uniform_real_distribution<float> random_y(0, size.Y -
                                                          (m_MaxQuadSize / 2));
    std::uniform_real_distribution<float> random_color_channel(0.0f, 1.0f);

    // for quads
    std::uniform_real_distribution<float> random_size(m_MinQuadSize,
                                                      m_MaxQuadSize);
    ...
    for (int i = 0; i < m_QuadCount; i++) {
      ...
      info.Color = {random_color_channel(rng), random_color_channel(rng),
  ...
  virtual void Render(Nexus::Time time) override {
    ...
    for (const auto &quad : m_Quads) {
      Nexus::Graphics::Rectangle rect(quad.Position.x, quad.Position.y,
  ...
  virtual void RenderUI() override {
    ...
    if (ImGui::DragFloat("Max Quad Size", &m_MaxQuadSize, 1.0f, 50.0f,
                         250.0f)) {
      GenerateShapes();
    }
  }
