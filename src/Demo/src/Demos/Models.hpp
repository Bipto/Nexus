...
public:
  ...
  virtual void Load() override {
    ...
    m_Model =
        factory.CreateFrom3DModelFile(Nexus::FileSystem::GetFilePathAbsolute(
            "resources/demo/models/survival_backpack_2/"
            "survival_backpack_2.fbx"));

    m_DiffuseMap = m_GraphicsDevice->CreateTexture2D(
        Nexus::FileSystem::GetFilePathAbsolute(
            "resources/demo/models/survival_backpack_2/1001_albedo.jpg"),
        true);
    m_NormalMap = m_GraphicsDevice->CreateTexture2D(
        Nexus::FileSystem::GetFilePathAbsolute(
            "resources/demo/models/survival_backpack_2/1001_normal.png"),
        true);
    m_SpecularMap = m_GraphicsDevice->CreateTexture2D(
        Nexus::FileSystem::GetFilePathAbsolute(
            "resources/demo/models/survival_backpack_2/specular.jpg"),
        true);
    ...
    m_CameraUniformBuffer =
        m_GraphicsDevice->CreateUniformBuffer(cameraUniformBufferDesc, nullptr);

    Nexus::Graphics::BufferDescription transformUniformBufferDesc;
    transformUniformBufferDesc.Size =
        sizeof(VB_UNIFORM_TRANSFORM_DEMO_LIGHTING);
    transformUniformBufferDesc.Usage = Nexus::Graphics::BufferUsage::Dynamic;
    m_TransformUniformBuffer = m_GraphicsDevice->CreateUniformBuffer(
        transformUniformBufferDesc, nullptr);
  ...
  virtual void Render(Nexus::Time time) override {
    ...
    m_TransformUniforms.Transform =
        glm::translate(glm::mat4(1.0f), {0.0f, 0.0f, 10.0f}) *
        glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), {0.0f, 1.0f, 0.0f});
    m_TransformUniformBuffer->SetData(&m_TransformUniforms,
                                      sizeof(m_TransformUniforms));

    m_CommandList->Begin();
    m_CommandList->SetPipeline(m_Pipeline);
    m_CommandList->SetRenderTarget(Nexus::Graphics::RenderTarget(
        m_GraphicsDevice->GetPrimaryWindow()->GetSwapchain()));
    ...
    m_CommandList->ClearColorTarget(
    ...
    m_ResourceSet->WriteCombinedImageSampler(m_DiffuseMap, m_Sampler,
                                             "diffuseMapSampler");
    m_ResourceSet->WriteCombinedImageSampler(m_NormalMap, m_Sampler,
                                             "normalMapSampler");
    m_ResourceSet->WriteCombinedImageSampler(m_SpecularMap, m_Sampler,
                                             "specularMapSampler");
    ...
    for (auto mesh : meshes) {
      ...
      auto indexCount =
          mesh->GetIndexBuffer()->GetDescription().Size / sizeof(unsigned int);
    ...
    m_Camera.Update(m_Window->GetWindowSize().X, m_Window->GetWindowSize().Y,
                    time);
...
private:
  void CreatePipeline() {
    ...
    pipelineDescription.VertexModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/models.vert.glsl",
            Nexus::Graphics::ShaderStage::Vertex);
    pipelineDescription.FragmentModule =
        m_GraphicsDevice->CreateShaderModuleFromSpirvFile(
            "resources/demo/shaders/models.frag.glsl",
    ...
    pipelineDescription.ColourTargetSampleCount =
        m_GraphicsDevice->GetPrimaryWindow()
            ->GetSwapchain()
            ->GetSpecification()
            .Samples;
