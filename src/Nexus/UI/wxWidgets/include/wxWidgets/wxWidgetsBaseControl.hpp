...
namespace Nexus::UI
{
	class wxWidgetsBaseControl : public IControl
	{
	  public:
		virtual ~wxWidgetsBaseControl() = default;

	  protected:
		template<typename InstantiateType, typename ReturnType, typename... Args>
		ReturnType *AddChild(Args &&...args)
		{
			static_assert(std::derived_from<InstantiateType, ReturnType>, "InstantiateType must derive from ReturnType");
