...
namespace Nexus
{
	/// @brief A class that repsents a wrapper around some kind of handle, e.g. an OpenGL texture
	...
	class AutoRelease
	{
	  public:
		...
		/// @brief A constructor taking in a handle, creates a default deleter, if one is available. Only available if the deleter is default
		/// constructible.
		/// @tparam D Template of the deleter, used to deduce if the deleter is default constructible
		...
		/// @brief A constructor taking in handle of the resource and a deleter to clean it up
		...
		/// @brief Copy construction of this object is not supported, as there can only be one owner of the handle
		/// @param Another AutoRelease object to copy
		AutoRelease(const AutoRelease &) = delete;

		/// @brief Copy assignment of this object is not supported, as there can only be one owner of the handle
		/// @param Another AutoRelease object to copy
		AutoRelease &operator=(const AutoRelease &) = delete;

		/// @brief Move constructor, taking in an r-value reference to the object to take ownership of
		/// @param other The object to take ownership of
		AutoRelease(AutoRelease &&other) noexcept;

		/// @brief Move assignment operator, taking in an r-value reference to the object to take ownership of
		/// @param other The object to take ownership of
		...
		/// @brief A method that returns a const reference to the handle being stored within this instance
		/// @return A const reference to the handle
		const Handle &Get() const noexcept;

		/// @brief A method that returns a reference to the deleter being stored within this instance
		/// @return A reference to the deleter
		Deleter &GetDeleter() noexcept;

		/// @brief A method that returns a const reference to the deleter being stored within this instance
		/// @return A const reference to the deleter
		const Deleter &GetDeleter() const noexcept;

		/// @brief An explicit bool operator to check whether the handle held by this instance is valid
		explicit operator bool() const noexcept;

		/// @brief A method that checks whether the handle held by this instance is valid
		...
		/// @tparam H H A type to check whether the reference operator should be included
		/// @return A reference to the handle
		template<typename H = Handle>
			requires std::is_pointer_v<H>
		auto &operator*() const noexcept;

		/// @brief A function that swaps two AutoRelease objects, for compatibility with std::swap
