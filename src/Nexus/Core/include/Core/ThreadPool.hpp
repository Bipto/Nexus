...
namespace Nexus
{
    /// @brief Class representing a set of threads and tasks that can be submitted to
    /// them
    class ThreadPool
    {
      public:
        /// @brief Constructor for ThreadPool, taking in the name, number of threads
        /// and maximum number of queued functions
        /// @param name The name of the thread pool for debugging, name will be
        ...
        /// @brief A function that waits until all work in the pool has been
        /// completed
        void WaitUntilIdle();

        /// @brief A function that temporarily pauses all execution within the thread
        /// pool
        void Pause();
      ...
      private:
        /// @brief A function that is internally called by the thread pool to execute
        /// queued functions
        void WorkerLoop();
      ...
      private:
        ...
        /// @brief A condition variable to synchronise sharing of work between
        /// threads
        ...
        /// @brief An integer containing the number of jobs currently being executed
        /// by the thread pool
        std::atomic<size_t> m_ActiveJobs{0};

        /// @brief The maximum number of threads that can be queued by the thread
        /// pool
