...
namespace Nexus
{
	...
	/// @brief A class that stores a size in bytes and allows converting to/from different units
	class MemorySize
	{
	  public:
		/// @brief A constructor that creates a MemorySize with a given size and unit
		/// @param value The value that should be represented, will be converted into bytes internally
		/// @param unit The unit that the size in represented in, used to convert to bytes internally
		...
		/// @brief An operator that implements all 5 comparison operators for this object
