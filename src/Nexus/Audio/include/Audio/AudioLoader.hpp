...
namespace Nexus::Audio
{
	...
	class NX_AUDIO_API AudioLoader
	{
	  public:
		...
		static std::shared_ptr<AudioBuffer> LoadWavFile(const std::string &filepath, AudioDevice *device);
		...
		/// @param device The audio device to use to create the audio buffer
		/// @return A created audio buffer containing the data loaded from the file
		static std::shared_ptr<AudioBuffer> LoadMp3File(const std::string &filepath, AudioDevice *device);
	};
};	  // namespace Nexus::Audio