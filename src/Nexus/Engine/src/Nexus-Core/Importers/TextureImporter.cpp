...
namespace
{
	...
	{
		bool isHdri = static_cast<bool>(stbi_is_hdr_from_memory(reinterpret_cast<const stbi_uc *>(data), static_cast<int>(length)));
		if (isHdri)
		{
			return Nexus::PixelType::PixelFloat;
		}

		bool is16Bit = static_cast<bool>(stbi_is_16_bit_from_memory(reinterpret_cast<const stbi_uc *>(data), static_cast<int>(length)));
		if (is16Bit)
		{
	...
	std::expected<Nexus::TextureData, std::string> LoadStbiImageDataToBuffer(const stbi_uc			*data,
																			 int					 length,
																			 ...
																			 std::optional<uint32_t> desiredChannels)
	{
		...
		switch (pixelType)
		{
			...
			{
				std::unique_ptr<float, void (*)(void *)> imageData(stbi_loadf_from_memory(data, length, &width, &height, &channels, targetChannels),
																   stbi_image_free);
				...
				int actualChannels = desiredChannels.has_value() ? *desiredChannels : channels;

				std::vector<std::byte> pixelData(static_cast<size_t>(width * height * actualChannels * sizeof(float)));
			...
			case Nexus::PixelType::Pixel16Bit:
			{
				std::unique_ptr<stbi_us, void (*)(void *)> imageData(
					stbi_load_16_from_memory(data, length, &width, &height, &channels, targetChannels),
					stbi_image_free);
				...
				std::vector<std::byte> pixelData(static_cast<size_t>(width * height * actualChannels * sizeof(uint16_t)));
				memcpy(pixelData.data(), imageData.get(), pixelData.size());
			...
			{
				std::unique_ptr<stbi_uc, void (*)(void *)> imageData(stbi_load_from_memory(data, length, &width, &height, &channels, targetChannels),
																	 stbi_image_free);
				...
				std::vector<std::byte> pixelData(static_cast<size_t>(width * height * actualChannels * sizeof(uint8_t)));
				memcpy(pixelData.data(), imageData.get(), pixelData.size());
			...
			default: return std::unexpected("Failed to find a valid pixel type");
		}
	}
}	 // namespace
...
namespace Nexus
{
	...
	std::expected<TextureInfo, std::string> TextureImporter::GetTextureInfoFromMemory(std::span<std::byte> buffer) const
	{
		int width = 0, height = 0, channels = 0;
		int result =
			stbi_info_from_memory(reinterpret_cast<const stbi_uc *>(buffer.data()), static_cast<int>(buffer.size()), &width, &height, &channels);

		if (result)
		{
			PixelType pixelType = GetPixelType(reinterpret_cast<const stbi_uc *>(buffer.data()), static_cast<int>(buffer.size()));
			return TextureInfo {
				.Width	  = static_cast<uint32_t>(width),
				.Height	  = static_cast<uint32_t>(height),
