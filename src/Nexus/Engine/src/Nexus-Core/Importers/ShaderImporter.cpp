		...
		const std::string						 &debugName) const
	{
		...
		if (Graphics::IsBinaryShaderFormat(shaderInputDescription.ShaderLanguage))
		{
			return loader->LoadBytes(path).and_then(
				...
				{ return CompileBinaryStep(shaderData, shaderInputDescription, shaderOutputDescription, environment, stage, debugName); });
		}
		...
		else
		{
			return loader->LoadString(path)
				...
				.and_then([&](const std::string &shaderText)
						  { return CompileTextStep(shaderText, shaderInputDescription, shaderOutputDescription, environment, stage, debugName); });
		}
	}

	std::expected<std::string, std::string> ShaderImporter::PreprocessStep(IResourceLoader				  *loader,
