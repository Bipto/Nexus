...
namespace Nexus
{
    std::expected<ShaderCompilationResult, std::string> ShaderImporter::
        LoadAndCompileShader(
            ...
            Graphics::GraphicsAPIInfo environment, Graphics::ShaderStage stage,
        ...
        ) const
    {
        ...
        if (Graphics::IsBinaryShaderFormat(shaderInputDescription.ShaderLanguage))
        {
            return loader->LoadBytes(path).and_then(
                [&](const std::vector<std::byte> &shaderData) {
                    return CompileBinaryStep(
                        shaderData, shaderInputDescription, shaderOutputDescription,
                        environment, stage, debugName
                    );
                }
            );
        }
            ...
            return loader->LoadString(path)
                .and_then([&](const std::string &shaderText) {
                    return PreprocessStep(
                        loader, shaderPathString, shaderText,
                        shaderIncludeDirectories
                    );
                })
                .and_then([&](const std::string &shaderText) {
                    return CompileTextStep(
                        shaderText, shaderInputDescription, shaderOutputDescription,
                        environment, stage, debugName
                    );
                });
        }
    ...
    std::expected<std::string, std::string> ShaderImporter::PreprocessStep(
        IResourceLoader *loader, const std::string &shaderPath,
        const std::string &shaderText,
        const std::vector<std::string> &shaderIncludeDirectories
    ) const
    {
        ShaderPreprocessor preprocessor(loader);
        return preprocessor.PreprocessShader(
            shaderPath, shaderText, shaderIncludeDirectories
        );
    }
    ...
    std::expected<ShaderCompilationResult, std::string> ShaderImporter::
        CompileBinaryStep(
            std::span<const std::byte> shaderBinary,
            const ShaderCompilationInputDescription &shaderInputDescription,
            const ShaderCompilationOutputDescription &shaderOutputDescription,
            Graphics::GraphicsAPIInfo environment, Graphics::ShaderStage stage,
            const std::string &debugName
