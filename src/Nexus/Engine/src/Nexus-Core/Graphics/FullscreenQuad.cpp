...
namespace Nexus::Graphics
{
    ...
    Nexus::Graphics::VertexBufferLayout FullscreenQuad::GetVertexBufferLayout()
    {
        if (m_HasUV)
        {
            return Nexus::Graphics::VertexBufferLayout({{Nexus::Graphics::ShaderDataType::R32G32B32_SFloat, "TEXCOORD"},
                                                       ...
                                                       sizeof(glm::vec3) + sizeof(glm::vec2), StepRate::Vertex);
        }
        else
        {
    ...
    void FullscreenQuad::CreateWithUV()
    {
        ...
        m_VertexBuffer =
            Utils::CreateFilledVertexBuffer(vertices.data(), vertices.size() * sizeof(VertexPositionTexCoord),
                                            sizeof(VertexPositionTexCoord), m_GraphicsDevice, m_CommandQueue);

        m_IndexBuffer = Utils::CreateFilledIndexBuffer(indices.data(), indices.size() * sizeof(uint32_t),
                                                       sizeof(uint32_t), m_GraphicsDevice, m_CommandQueue);
    }
    ...
    void FullscreenQuad::CreateWithoutUV()
    {
        ...
        m_VertexBuffer = Utils::CreateFilledVertexBuffer(vertices.data(), vertices.size() * sizeof(VertexPosition),
