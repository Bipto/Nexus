...
{
    RenderDocAPI::RenderDocAPI(const std::string &libraryPath)
        : m_Filepath(libraryPath)
    {
        m_SharedLibrary = std::unique_ptr<Utils::SharedLibrary>(
            Platform::LoadSharedLibrary(m_Filepath)
        );
        if (m_SharedLibrary)
        {
            auto RENDERDOC_GetAPI =
                (pRENDERDOC_GetAPI)(m_SharedLibrary->LoadSymbol("RENDERDOC_GetAPI"));
            if (RENDERDOC_GetAPI)
            {
                int ret =
                    RENDERDOC_GetAPI(eRENDERDOC_API_Version_1_6_0, (void **)&m_API);
                if (ret == 1 && m_API)
                {
                    m_Loaded = true;
                }
