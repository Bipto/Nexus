...
namespace Nexus
{
    std::expected<std::vector<std::byte>, std::string> MemoryResourceLoader::LoadBytes(std::string_view path) const
    {
        ...
        else
        {
            return std::unexpected(
                std::format("Failed to find data mounted to given path: {}", filesystemPath.string()));
        }
    }
    ...
    std::expected<void, std::string> MemoryResourceLoader::MountBinaryFile(std::string_view path, bool overwrite,
                                                                           const std::vector<std::byte> &data)
    {
        ...
        {
            return std::unexpected(std::format("Attempting to mount file to path, however a file already "
    ...
    std::expected<void, std::string> MemoryResourceLoader::MountTextFile(std::string_view path, bool overwrite,
