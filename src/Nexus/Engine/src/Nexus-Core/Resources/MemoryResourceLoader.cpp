...
namespace Nexus
{
    std::expected<std::vector<std::byte>, std::string> MemoryResourceLoader::
        LoadBytes(std::string_view path) const
    {
        ...
        else
        {
            return std::unexpected(
                std::format(
                    "Failed to find data mounted to given path: {}",
    ...
    std::expected<std::string, std::string> MemoryResourceLoader::LoadString(
        std::string_view path
    ) const
    {
        return LoadBytes(path).and_then(
            [](
    ...
    std::expected<void, std::string> MemoryResourceLoader::MountBinaryFile(
    ...
    )
    {
        ...
        if (m_MountedFiles.contains(filepath) && !overwrite)
        {
            return std::unexpected(
                std::format(
                    "Attempting to mount file to path, however a file already "
    ...
    std::expected<void, std::string> MemoryResourceLoader::MountTextFile(
        std::string_view path, bool overwrite, const std::string &data
    )
    {
