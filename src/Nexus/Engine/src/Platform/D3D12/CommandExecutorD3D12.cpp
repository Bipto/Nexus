...
namespace Nexus::Graphics
{
        ...
        : m_Device(device)
    {
        ...
        if (m_PixModule)
        {
            m_PIXBeginEvent = (PIXBeginEventFn)GetProcAddress(
                m_PixModule, "PIXBeginEventOnCommandList"
            );
            m_PIXEndEvent = (PIXEndEventFn)GetProcAddress(
                m_PixModule, "PIXEndEventOnCommandList"
            );
            m_PIXSetMarker = (PIXSetMarkerFn)GetProcAddress(
                m_PixModule, "PIXSetMarkerOnCommandList"
            );
        }
        else
        {
    ...
    void CommandExecutorD3D12::ExecuteCommands(
        ICommandList *commandList, IGraphicsDevice *device
    )
    {
        const std::vector<std::unique_ptr<IGraphicsCommand>> &commands =
    ...
    void CommandExecutorD3D12::SetCommandList(
    ...
    void CommandExecutorD3D12::FlushReadbacks(IGraphicsDevice *device)
    {
        ...
        std::vector<D3D12ReadbackBufferCopyOperation> readbackCopies =
            m_ReadbackCopies;
        ...
        {
            const uint32_t copyPerRow = std::min(
                readbackCopy.SourceRowPitch, readbackCopy.DestinationRowPitch
            );

            uint8_t *srcData = readbackCopy.ReadbackBuffer->Map();

            std::vector<uint8_t> pixels(
                readbackCopy.NumRows * readbackCopy.DestinationRowPitch, 0
            );
            ...
            for (uint32_t y = 0; y < readbackCopy.NumRows; y++)
            {
                ...
                uint8_t *dstRow =
                    pixels.data() + y * readbackCopy.DestinationRowPitch;
    ...
    )
    {
        ...
        if (m_CurrentlyBoundPipeline.IsValid() &&
            m_CurrentlyBoundPipeline->GetType() == PipelineType::Graphics)
        {
            const GraphicsPipelineD3D12 *pipeline =
                m_CurrentlyBoundPipeline.AsDerived<const GraphicsPipelineD3D12>();
            const DeviceBufferD3D12 *d3d12VertexBuffer =
                command.View.BufferHandle.AsDerived<const DeviceBufferD3D12>();
            const auto &bufferLayout =
                pipeline->GetPipelineDescription().Layouts.at(command.Slot);

            D3D12_VERTEX_BUFFER_VIEW bufferView = {};
            bufferView.BufferLocation =
                d3d12VertexBuffer->GetHandle()->GetGPUVirtualAddress() +
                command.View.Offset;
            bufferView.SizeInBytes = command.View.Size;
            bufferView.StrideInBytes = pipeline->GetPipelineDescription()
                                           .Layouts.at(command.Slot)
                                           .GetStride();
    ...
    )
    {
        ...
        const DeviceBufferD3D12 *d3d12IndexBuffer =
            command.View.BufferHandle.AsDerived<const DeviceBufferD3D12>();

        D3D12_INDEX_BUFFER_VIEW indexBufferView = {};
        indexBufferView.BufferLocation =
            d3d12IndexBuffer->GetHandle()->GetGPUVirtualAddress() +
            command.View.Offset;
        indexBufferView.SizeInBytes = command.View.Size;
        indexBufferView.Format =
            D3D12::GetD3D12IndexBufferFormat(command.View.BufferFormat);

        m_CommandList->IASetIndexBuffer(&indexBufferView);
    }

    void CommandExecutorD3D12::ExecuteCommand(
        PipelineHandle command, IGraphicsDevice *device
    )
    {
    ...
    )
    {
        ...
        m_CommandList->DrawInstanced(
            command.VertexCount, command.InstanceCount, command.VertexStart,
    ...
    )
    {
        ...
        m_CommandList->DrawIndexedInstanced(
            command.IndexCount, command.InstanceCount, command.IndexStart,
    ...
    )
    {
        ...
        if (m_CurrentlyBoundPipeline.IsValid() &&
            m_CurrentlyBoundPipeline->GetType() == PipelineType::Graphics)
        {
            if (const DeviceBufferD3D12 *indirectBuffer =
                    command.IndirectBuffer.AsDerived<const DeviceBufferD3D12>())
            {
                Microsoft::WRL::ComPtr<ID3D12Resource2> indirectBufferHandle =
                    indirectBuffer->GetHandle();

                Microsoft::WRL::ComPtr<ID3D12CommandSignature> signature =
                    GetOrCreateIndirectCommandSignature(
                        D3D12_INDIRECT_ARGUMENT_TYPE_DRAW, command.Stride
                    );

                m_CommandList->ExecuteIndirect(
                    signature.Get(), command.DrawCount, indirectBufferHandle.Get(),
    ...
    )
    {
            ...
            m_CurrentlyBoundPipeline->GetType() == PipelineType::Graphics)
        {
                    ...
                    command.IndirectBuffer.AsDerived<const DeviceBufferD3D12>())
            {
                ...
                Microsoft::WRL::ComPtr<ID3D12CommandSignature> signature =
                    GetOrCreateIndirectCommandSignature(
                        D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED, command.Stride
                    );
    ...
    )
    {
        ...
        m_CommandList->Dispatch(
            command.WorkGroupCountX, command.WorkGroupCountY, command.WorkGroupCountZ
        );
    }
    ...
    )
    {
                ...
                command.IndirectBuffer.AsDerived<const DeviceBufferD3D12>())
        {
            ...
            Microsoft::WRL::ComPtr<ID3D12CommandSignature> signature =
                GetOrCreateIndirectCommandSignature(
                    D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH, command.Stride
                );

            m_CommandList->ExecuteIndirect(
                signature.Get(), 1, indirectBufferHandle.Get(), command.Offset,
                nullptr, 0
            );
        }
    }
    ...
    )
    {
...

        {
                    ...
                    command.IndirectBuffer.AsDerived<const DeviceBufferD3D12>())
            {
                ...
                Microsoft::WRL::ComPtr<ID3D12CommandSignature> signature =
                    GetOrCreateIndirectCommandSignature(
                        D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH_MESH, command.Stride
                    );
    ...
    {
        Nexus::Graphics::PipelineType pipelineType =
            m_CurrentlyBoundPipeline->GetType();
    ...
    )
    {
        ...
        float clearColor[] = {
            command.Colour.Red, command.Colour.Green, command.Colour.Blue,
            command.Colour.Alpha
        };
        ...
        if (m_DepthHandle.ptr)
        {
            ...
            if (command.Rect.has_value())
            {
                ...
                m_CommandList->ClearDepthStencilView(
                    m_DepthHandle, clearFlags, command.Value.Depth,
    ...
    void CommandExecutorD3D12::ExecuteCommand(
        const Viewport &command, IGraphicsDevice *device
    )
    {
        D3D12_VIEWPORT vp = {};
    ...
    void CommandExecutorD3D12::ExecuteCommand(
        const Scissor &command, IGraphicsDevice *device
    )
    {
        RECT rect = {};
        rect.left = command.X;
    ...
    {
        if (const TextureD3D12 *const source =
                command.Source.AsDerived<const TextureD3D12>())
        {
            uint32_t sourceSubresource = Utils::CalculateSubresource(
                command.SourceMipLevel, command.SourceArrayLayer,
                source->GetMipLevels()
            );
            Microsoft::WRL::ComPtr<ID3D12Resource2> sourceHandle =
                source->GetHandle();

            if (const TextureD3D12 *dest =
                    command.Destination.AsDerived<const TextureD3D12>())
            {
                uint32_t destinationSubresource = Utils::CalculateSubresource(
                    command.DestinationMipLevel, command.DestinationArrayLayer,
                    dest->GetMipLevels()
                );
                Microsoft::WRL::ComPtr<ID3D12Resource2> destHandle =
                    dest->GetHandle();

                PixelFormat destFormat = dest->GetPixelFormat();

                m_CommandList->ResolveSubresource(
                    destHandle.Get(), destinationSubresource, sourceHandle.Get(),
    ...
    {
        if (const TimingQueryD3D12 *queryD3D12 =
                command.Query.AsDerived<const TimingQueryD3D12>())
        {
            Microsoft::WRL::ComPtr<ID3D12QueryHeap> heap =
                queryD3D12->GetQueryHeap();
    ...
    {
        const DeviceBufferD3D12 *source =
            command.BufferCopy.Source.AsDerived<const DeviceBufferD3D12>();
        const DeviceBufferD3D12 *dest =
            command.BufferCopy.Destination.AsDerived<const DeviceBufferD3D12>();
        ...
        if (source && dest)
        {
            for (const auto &copy : command.BufferCopy.Copies)
            {
                m_CommandList->CopyBufferRegion(
                    dest->GetHandle().Get(), copy.WriteOffset,
    ...
    {
        GraphicsDeviceD3D12 *deviceD3D12 = (GraphicsDeviceD3D12 *)device;
        Microsoft::WRL::ComPtr<ID3D12Device9> nativeDevice =
            deviceD3D12->GetD3D12Device();

        const TextureD3D12 *texture =
            command.BufferTextureCopy.Texture.AsDerived<const TextureD3D12>();
        ...
        textureBounds.right = command.BufferTextureCopy.TextureOffset.X +
                              command.BufferTextureCopy.TextureExtent.Width;
        textureBounds.top = command.BufferTextureCopy.TextureOffset.Y;
        textureBounds.bottom = command.BufferTextureCopy.TextureOffset.Y +
        ...
        nativeDevice->GetCopyableFootprints(
            &resourceDesc, subresourceIndex, 1,
            command.BufferTextureCopy.BufferOffset, &footprint, &numRows,
            &rowSizeInBytes, &totalBytes
        );

        DeviceBufferHandle stagingBuffer =
            CreateStagingBuffer(totalBytes, true, device);
        DeviceBufferHandle destHandle = command.BufferTextureCopy.BufferHandle;
        DeviceBufferD3D12 *sourceBufferD3D12 =
            destHandle.AsDerived<DeviceBufferD3D12>();
        const DeviceBufferD3D12 *stagingBufferD3D12 =
            stagingBuffer.AsDerived<const DeviceBufferD3D12>();
        Microsoft::WRL::ComPtr<ID3D12Resource> bufferHandle =
            stagingBufferD3D12->GetHandle();
        ...
        uint32_t rowSize = command.BufferTextureCopy.TextureExtent.Width *
                           GetPixelFormatSizeInBytes(texture->GetPixelFormat());
        ...
        const uint32_t zOffset = texture->GetType() == TextureType::Texture3D
                                     ? command.BufferTextureCopy.TextureOffset.Z
                                     : 0;

        m_CommandList->CopyTextureRegion(
            &dstLocation, command.BufferTextureCopy.TextureOffset.X,
    ...
    )
    {
        ...
        const TextureD3D12 *texture =
            command.TextureBufferCopy.Texture.AsDerived<const TextureD3D12>();
        ...
        bool arrayedTexture =
            command.TextureBufferCopy.Texture->GetType() == TextureType::Texture3D ||
            command.TextureBufferCopy.Texture->GetType() == TextureType::TextureCube;
        uint32_t subresourceIndex = Utils::CalculateSubresource(
            command.TextureBufferCopy.MipLevel,
        ...
        textureBounds.right = command.TextureBufferCopy.TextureOffset.X +
                              command.TextureBufferCopy.TextureExtent.Width;
        textureBounds.top = command.TextureBufferCopy.TextureOffset.Y;
        textureBounds.bottom = command.TextureBufferCopy.TextureOffset.Y +
        ...
        nativeDevice->GetCopyableFootprints(
            &resourceDesc, subresourceIndex, 1,
            command.TextureBufferCopy.BufferOffset, &footprint, &numRows,
            &rowSizeInBytes, &totalBytes
        );
        ...
        m_CommandList->CopyTextureRegion(
            &dstLocation, 0, 0, 0, &srcLocation, &textureBounds
        );

        size_t tightPitch =
            command.TextureBufferCopy.TextureExtent.Width *
            GetPixelFormatSizeInBytes(texture->GetDescription().Format);

        D3D12ReadbackBufferCopyOperation &readbackOperation =
            m_ReadbackCopies.emplace_back();
    ...
    )
    {
        ...
        const TextureD3D12 *srcTexture =
            command.TextureCopy.Source.AsDerived<const TextureD3D12>();
        const TextureD3D12 *dstTexture =
            command.TextureCopy.Destination.AsDerived<const TextureD3D12>();
        ...
        bool srcArrayedTexture = srcTexture->GetType() == TextureType::Texture3D ||
                                 srcTexture->GetType() == TextureType::TextureCube;
        uint32_t srcSubresourceIndex = Utils::CalculateSubresource(
            command.TextureCopy.SourceMipLevel,
        ...
        bool dstArrayedTexture = dstTexture->GetType() == TextureType::Texture3D ||
                                 dstTexture->GetType() == TextureType::TextureCube;
        ...
        TextureLayout srcLayout = srcTexture->GetTextureLayout(
            command.TextureCopy.SourceOffset.Z, command.TextureCopy.SourceMipLevel
        );
        D3D12_RESOURCE_STATES srcResourceState =
            D3D12::GetTextureResourceState(srcLayout);
    ...
    {
        float blendFactor[4] = {
            command.BlendFactor.Red, command.BlendFactor.Green,
            command.BlendFactor.Blue, command.BlendFactor.Alpha
        };
    ...
    )
    {
        for (const auto &accelerationStructureBuildDesc : command.BuildDescriptions)
        {
            ...
            D3D12::GetD3D12AccelerationStructureInputs(
                accelerationStructureBuildDesc, inputs, geometry
            );

            D3D12_GPU_VIRTUAL_ADDRESS srcAddress = 0;
            D3D12_GPU_VIRTUAL_ADDRESS destAddress = 0;
            D3D12_GPU_VIRTUAL_ADDRESS scratchAddress =
                accelerationStructureBuildDesc.ScratchBuffer;

            if (accelerationStructureBuildDesc.Source.IsValid())
            {
                srcAddress =
                    accelerationStructureBuildDesc.Source->GetDeviceAddress(0);
            }

            if (accelerationStructureBuildDesc.Destination.IsValid())
            {
                destAddress =
                    accelerationStructureBuildDesc.Destination->GetDeviceAddress(0);
            }
    ...
    )
    {
        ...
        bool isGraphics =
            m_CurrentlyBoundPipeline->GetType() != PipelineType::Compute;

        m_CurrentlyBoundResourceSet->SetPushConstants(
            command.Name, command.Data.data(), command.Offset, command.Data.size(),
            isGraphics, m_CommandList
        );
    }
        ...
        if (d3d12Features.SupportsEnhancedBarriers)
        {
            ...
            for (const auto &memoryBarrier : command.MemoryBarriers)
            {
                D3D12_BARRIER_SYNC beforeSync =
                    D3D12::GetBarrierSync(memoryBarrier.BeforeStage);
                D3D12_BARRIER_SYNC afterSync =
                    D3D12::GetBarrierSync(memoryBarrier.AfterStage);
                D3D12_BARRIER_ACCESS beforeAccess =
                    D3D12::GetBarrierAccess(memoryBarrier.BeforeAccess);
                D3D12_BARRIER_ACCESS afterAccess =
                    D3D12::GetBarrierAccess(memoryBarrier.AfterAccess);
            ...
            {
                const TextureD3D12 *texture =
                    textureBarrier.Texture.AsDerived<const TextureD3D12>();
                Microsoft::WRL::ComPtr<ID3D12Resource2> handle =
                    texture->GetHandle();

                D3D12_BARRIER_SYNC beforeSync =
                    D3D12::GetBarrierSync(textureBarrier.BeforeStage);
                D3D12_BARRIER_SYNC afterSync =
                    D3D12::GetBarrierSync(textureBarrier.AfterStage);
                D3D12_BARRIER_ACCESS beforeAccess =
                    D3D12::GetBarrierAccess(textureBarrier.BeforeAccess);
                D3D12_BARRIER_ACCESS afterAccess =
                    D3D12::GetBarrierAccess(textureBarrier.AfterAccess);

                D3D12_BARRIER_LAYOUT afterLayout =
                    D3D12::GetBarrierLayout(textureBarrier.Layout);

                for (uint32_t arrayLayer =
                         textureBarrier.TextureSubresourceRange.BaseArrayLayer;
                     arrayLayer <
                     textureBarrier.TextureSubresourceRange.BaseArrayLayer +
                     ...
                     arrayLayer++)
                {
                    for (uint32_t mipLevel =
                             textureBarrier.TextureSubresourceRange.BaseMipLevel;
                         mipLevel <
                         textureBarrier.TextureSubresourceRange.BaseMipLevel +
                             textureBarrier.TextureSubresourceRange.LevelCount;
                         mipLevel++)
                    {
                        D3D12_BARRIER_LAYOUT beforeLayout = D3D12::GetBarrierLayout(
                            texture->GetTextureLayout(arrayLayer, mipLevel)
                        );
            ...
            for (const auto &bufferBarrier : command.BufferBarriers)
            {
                Ref<DeviceBufferD3D12> buffer =
                    std::dynamic_pointer_cast<DeviceBufferD3D12>(
                        bufferBarrier.Buffer
                    );
                Microsoft::WRL::ComPtr<ID3D12Resource2> handle = buffer->GetHandle();

                D3D12_BARRIER_SYNC beforeSync =
                    D3D12::GetBarrierSync(bufferBarrier.BeforeStage);
                D3D12_BARRIER_SYNC afterSync =
                    D3D12::GetBarrierSync(bufferBarrier.AfterStage);
                D3D12_BARRIER_ACCESS beforeAccess =
                    D3D12::GetBarrierAccess(bufferBarrier.BeforeAccess);
                D3D12_BARRIER_ACCESS afterAccess =
                    D3D12::GetBarrierAccess(bufferBarrier.AfterAccess);
            ...
            if (globalBarriers.size() > 0)
            {
                ...
                globalBarrierGroup.NumBarriers =
                    static_cast<UINT32>(globalBarriers.size());
            ...
            if (textureBarriers.size() > 0)
            {
                ...
                textureBarrierGroup.NumBarriers =
                    static_cast<UINT32>(textureBarriers.size());
            ...
            if (bufferBarriers.size() > 0)
            {
                ...
                bufferBarrierGroup.NumBarriers =
                     ...
                     arrayLayer++)
                {
                         ...
                         mipLevel++)
                    {
                        D3D12_RESOURCE_STATES beforeState =
                            D3D12::GetTextureResourceState(
                                texture->GetTextureLayout(arrayLayer, mipLevel)
                            );
                        D3D12_RESOURCE_STATES afterState =
                            D3D12::GetTextureResourceState(textureBarrier.Layout);
                        ...
                        uint32_t subresourceIndex = Utils::CalculateSubresource(
                            mipLevel, arrayLayer, texture->GetDescription().MipLevels
                        );

                        D3D12_RESOURCE_BARRIER &barrier =
                            resourceBarriers.emplace_back();
        ...
        for (const TextureBarrierDesc &textureBarrier : command.TextureBarriers)
        {
                 ...
                 arrayLayer++)
            {
                for (uint32_t mipLevel =
                     ...
                     mipLevel++)
                {
                    textureD3D12->SetTextureLayout(
                        arrayLayer, mipLevel, textureBarrier.Layout
                    );
                }
            }
        }
    ...
    )
    {
        ...
        if (framebuffer->GetSampleCount() > 1)
        {
            for (size_t textureIndex = 0;
                 textureIndex < framebuffer->GetColorTextureCount(); textureIndex++)
            {
                std::optional<FramebufferColourAttachmentDescription>
                    colourAttachmentDescOpt =
                    ...
                    if (colourAttachmentDesc.ResolveAttachment.has_value())
                    {
                             ...
                             layerIndex++)
                        {
                            ...
                            uint32_t sourceArrayIndex =
                                colourAttachmentDesc.ColourAttachment
                                    .BaseArrayLayer +
                                layerIndex;
                            uint32_t destArrayIndex =
                                resolveAttachmentDesc.BaseArrayLayer + layerIndex;

                            TextureLayout sourceLayout =
                                colourAttachmentDesc.ColourAttachment.TargetTexture
                                    ->GetTextureLayout(
                                        sourceArrayIndex,
                                        colourAttachmentDesc.ColourAttachment
                                            .MipLevel
                            ...
                            TextureLayout destLayout =
                                resolveAttachmentDesc.TargetTexture
                                    ->GetTextureLayout(
                                        destArrayIndex,
                                        resolveAttachmentDesc.MipLevel
                                    );
                            ...
                            // to resolve compatible layouts
                            {
                                ...
                                sourceBarrier.Texture =
                                    colourAttachmentDesc.ColourAttachment
                                        .TargetTexture;
                                sourceBarrier.BeforeAccess =
                                    BarrierAccess::ColourAttachmentWrite;
                                sourceBarrier.AfterAccess =
                                    BarrierAccess::ColourAttachmentRead;
                                sourceBarrier.BeforeStage =
                                    BarrierPipelineStage::ColourAttachmentOutput;
                                sourceBarrier.AfterStage =
                                    BarrierPipelineStage::Resolve;
                                sourceBarrier.Layout = TextureLayout::ResolveSrc;
                                sourceBarrier.TextureSubresourceRange = {
                                    .BaseMipLevel = colourAttachmentDesc
                                                        .ColourAttachment.MipLevel,
                                    .LevelCount = 1,
                                    .BaseArrayLayer = sourceArrayIndex,
                                    .LayerCount = 1
                                };
                                barrierGroup.TextureBarriers.emplace_back(
                                    sourceBarrier
                                );
                                ...
                                destBarrier.Texture =
                                    resolveAttachmentDesc.TargetTexture;
                                destBarrier.BeforeAccess =
                                    BarrierAccess::ColourAttachmentWrite;
                                destBarrier.AfterAccess =
                                    BarrierAccess::ColourAttachmentRead;
                                destBarrier.BeforeStage =
                                    BarrierPipelineStage::ColourAttachmentOutput;
                                destBarrier.AfterStage =
                                    BarrierPipelineStage::Resolve;
    ...
    void CommandExecutorD3D12::SetFramebuffer(
        FramebufferHandle framebuffer, IGraphicsDevice *device
    )
    {
        ...
        if (framebuffer.IsValid())
        {
            FramebufferD3D12 *framebufferD3D12 =
                framebuffer.AsDerived<FramebufferD3D12>();
            ...
            if (framebufferD3D12->HasDepthTexture())
            {
                m_CommandList->OMSetRenderTargets(
                    m_DescriptorHandles.size(), m_DescriptorHandles.data(), false,
                    &m_DepthHandle
                );
            }
            else
    ...
    Microsoft::WRL::ComPtr<ID3D12CommandSignature> CommandExecutorD3D12::
        GetOrCreateIndirectCommandSignature(
            D3D12_INDIRECT_ARGUMENT_TYPE type, size_t stride
        )
    {
        // element found in map
        if (m_IndirectCommandSignatures[type].find(stride) !=
            m_IndirectCommandSignatures[type].end())
        {
    ...
    void CommandExecutorD3D12::InsertResourceBarrier(
        const TextureBarrierDesc &command
    )
    {
        ...
        for (uint32_t arrayLayer = command.TextureSubresourceRange.BaseArrayLayer;
             arrayLayer < command.TextureSubresourceRange.BaseArrayLayer +
                              command.TextureSubresourceRange.LayerCount;
             arrayLayer++)
        {
            for (uint32_t mipLevel = command.TextureSubresourceRange.BaseMipLevel;
                 mipLevel < command.TextureSubresourceRange.BaseMipLevel +
                                command.TextureSubresourceRange.LevelCount;
                 mipLevel++)
            {
                D3D12_RESOURCE_STATES beforeState = D3D12::GetTextureResourceState(
                    texture->GetTextureLayout(arrayLayer, mipLevel)
                );
                D3D12_RESOURCE_STATES afterState =
                    D3D12::GetTextureResourceState(command.Layout);
    ...
    void CommandExecutorD3D12::InsertTextureBarrier(
        const TextureBarrierDesc &command
    )
    {
        TextureHandle textureHandle = command.Texture;
        ...
        if (command.TextureSubresourceRange.LayerCount ==
                texture->GetDescription().DepthOrArrayLayers &&
            command.TextureSubresourceRange.LevelCount ==
                texture->GetDescription().MipLevels)
        {
            for (uint32_t arrayLayer =
                     command.TextureSubresourceRange.BaseArrayLayer;
                 arrayLayer < command.TextureSubresourceRange.BaseArrayLayer +
                                  command.TextureSubresourceRange.LayerCount;
                 arrayLayer++)
            {
                for (uint32_t mipLevel =
                         command.TextureSubresourceRange.BaseMipLevel;
                     mipLevel < command.TextureSubresourceRange.BaseMipLevel +
                                    command.TextureSubresourceRange.LevelCount;
                     mipLevel++)
                {
                    TextureLayout subresourceLayout =
                        texture->GetTextureLayout(arrayLayer, mipLevel);
        ...
        if (!transitionEachSubresourceSeparately)
        {
            ...
            D3D12_BARRIER_LAYOUT afterLayout =
                D3D12::GetBarrierLayout(command.Layout);
            ...
            D3D12_BARRIER_SYNC beforeSync =
                D3D12::GetBarrierSync(command.BeforeStage);
            D3D12_BARRIER_SYNC afterSync = D3D12::GetBarrierSync(command.AfterStage);
            D3D12_BARRIER_ACCESS beforeAccess =
                D3D12::GetBarrierAccess(command.BeforeAccess);
            D3D12_BARRIER_ACCESS afterAccess =
                D3D12::GetBarrierAccess(command.AfterAccess);
            ...
            barrier.Subresources.FirstArraySlice =
                command.TextureSubresourceRange.BaseArrayLayer;
            barrier.Subresources.NumArraySlices =
                command.TextureSubresourceRange.LayerCount;
            barrier.Subresources.IndexOrFirstMipLevel =
                command.TextureSubresourceRange.BaseMipLevel;
            barrier.Subresources.NumMipLevels =
                command.TextureSubresourceRange.LevelCount;
            barrier.Subresources.FirstPlane = 0;
            barrier.Subresources.NumPlanes = 1;
        }
                 ...
                 arrayLayer++)
            {
                     ...
                     mipLevel++)
                {
                    D3D12_BARRIER_LAYOUT beforeLayout =
                        D3D12::GetBarrierLayout(testLayout);
                    D3D12_BARRIER_LAYOUT afterLayout =
                        D3D12::GetBarrierLayout(command.Layout);
                    ...
                    D3D12_BARRIER_SYNC beforeSync =
                        D3D12::GetBarrierSync(command.BeforeStage);
                    D3D12_BARRIER_SYNC afterSync =
                        D3D12::GetBarrierSync(command.AfterStage);
                    D3D12_BARRIER_ACCESS beforeAccess =
                        D3D12::GetBarrierAccess(command.BeforeAccess);
                    D3D12_BARRIER_ACCESS afterAccess =
                        D3D12::GetBarrierAccess(command.AfterAccess);
    ...
    )
    {
        ...
        upload ? description.Access = BufferMemoryAccess::Upload
               : description.Access = BufferMemoryAccess::Readback;
