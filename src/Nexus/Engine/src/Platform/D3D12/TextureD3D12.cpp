...
namespace Nexus::Graphics
{
        ...
        : ITexture(spec), m_Device(device)
    {
        ...
        NX_VALIDATE(
            spec.DepthOrArrayLayers >= 1, "Texture must have at least one mip level"
        );
        ...
        else
        {
            ...
            hr = allocator->CreateResource(
                &allocationDesc, &textureDesc, D3D12_RESOURCE_STATE_COMMON, nullptr,
        ...
        if (FAILED(hr))
        {
            ...
            std::string errorMessage =
                std::string("Failed to create texture: ") + error.ErrorMessage();
    ...
    TextureLayout TextureD3D12::GetTextureLayout(
    ...
    void TextureD3D12::SetTextureLayout(
    ...
    ) const
    {
        ...
        device->GetCopyableFootprints(
            &resourceDesc, subresourceIndex, 1, 0, &placedFootprint, &numRows,
