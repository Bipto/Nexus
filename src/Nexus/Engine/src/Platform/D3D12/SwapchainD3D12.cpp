...
namespace Nexus::Graphics
{
    ...
    {
        // we need to flush the swapchain to ensure that resources are not in use
        // when we attempt to delete them
        Flush();
    ...
    void SwapchainD3D12::SwapBuffers(const SwapchainPresentDescription &presentDesc)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            for (const auto &rect : presentDesc.PresentRects)
            {
                ...
                presentRect.left = rect.X;
                presentRect.top = rect.Y;
                presentRect.right = rect.Width;
                presentRect.bottom = rect.Height;
            }
    ...
    void SwapchainD3D12::ResizeBuffers()
    {
        ...
        // we need to release the buffers retrieved from the swapchain before new
        // ones can be created
        ReleaseBuffers();
