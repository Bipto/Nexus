    ...
    class ResourceSetD3D12 final : public IResourceSet
    {
      ...
      private:
        Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> m_SamplerDescriptorHeap =
            nullptr;
        Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> m_SRV_UAV_CBV_DescriptorHeap =
            nullptr;

        std::vector<ID3D12DescriptorHeap *> m_DescriptorHeapArray = {};

        std::map<std::string, std::vector<D3D12_CPU_DESCRIPTOR_HANDLE>>
            m_SamplerDescriptorHandles = {};
        std::map<std::string, std::vector<D3D12_CPU_DESCRIPTOR_HANDLE>>
            m_SRV_UAV_CBV_DescriptorHandles = {};
