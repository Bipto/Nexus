...
namespace Nexus::Graphics
{
    CommandQueueVk::CommandQueueVk(
        GraphicsDeviceVk *device, const CommandQueueDescription &description
    )
        : m_Device(device), m_Description(description)
    {
        m_Queue = Vk::GetDeviceQueue(device, description);
        device->SetObjectName(
            VK_OBJECT_TYPE_QUEUE, (uint64_t)m_Queue, description.DebugName.c_str()
        );
    ...
    SwapchainHandle CommandQueueVk::CreateSwapchain(const SwapchainDescription &spec)
    {
        ...
        context.GetPhysicalDeviceSurfaceSupportKHR(
            physicalDeviceVk->GetVkPhysicalDevice(), m_Description.QueueFamilyIndex,
            swapchain->GetSurface(), &presentSupport
        );

        if (!presentSupport)
        {
            throw std::runtime_error(
                "Device is unable to present to this swapchain"
            );
        }
    ...
    void CommandQueueVk::SubmitCommandList(
    ...
    )
    {
        ...
        for (uint32_t i = 0; i < numCommandLists; i++)
        {
            ...
            m_CommandExecutor->SetCommandBuffer(
                commandList->GetCurrentCommandBuffer()
            );
        ...
        NX_VALIDATE(
            Vk::SubmitQueue(
                m_Device, m_Queue, commandBuffers, waitDestStageMask, vulkanFence
            ) == VK_SUCCESS,
    ...
    CommandListHandle CommandQueueVk::CreateCommandList(
