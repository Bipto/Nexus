...
namespace Nexus::Graphics
{
    ...
    )
    {
        uint32_t count = 0;
        VkResult result =
            context.EnumerateInstanceExtensionProperties(nullptr, &count, nullptr);
    ...
    VkResult CreateDebugUtilsMessengerEXT(
        const GladVulkanContext &context, VkInstance instance,
        const VkDebugUtilsMessengerCreateInfoEXT *pCreateInfo,
        const VkAllocationCallbacks *pAllocator,
        VkDebugUtilsMessengerEXT *pDebugMessenger
    )
    {
        auto func = (PFN_vkCreateDebugUtilsMessengerEXT)context.GetInstanceProcAddr(
    ...
    std::vector<const char *> GetRequiredInstanceExtensions(bool debug)
    {
        std::vector<const char *> extensionNames =
            PlatformVk::GetRequiredExtensions();
    ...
    GraphicsAPI_Vk::GraphicsAPI_Vk(const GraphicsAPICreateInfo &createInfo)
        : m_CreateInfo(createInfo)
    {
        ...
        {
            NX_VALIDATE(
                CheckValidationLayerSupport(m_Context),
        ...
        NX_VALIDATE(
            m_Context.CreateInstance(&instanceCreateInfo, nullptr, &m_Instance) ==
                VK_SUCCESS,
            "Failed to create Vulkan instance"
        );

        Vk::GladLoaderData loadData = {
            .instance = m_Instance, .device = VK_NULL_HANDLE
        };
        gladLoadVulkanContextUserPtr(
            &m_Context, VK_NULL_HANDLE,
    ...
    GraphicsAPI_Vk::~GraphicsAPI_Vk()
    {
        if (m_CreateInfo.Debug)
        {
            DestroyDebugUtilsMessengerEXT(
                m_Context, m_Instance, m_DebugMessenger, nullptr
            );
        }
    ...
    Graphics::IGraphicsDevice *GraphicsAPI_Vk::CreateGraphicsDevice(
    ...
    void GraphicsAPI_Vk::SetupDebugMessenger()
    {
        ...
        NX_VALIDATE(
            CreateDebugUtilsMessengerEXT(
                m_Context, m_Instance, &createInfo, nullptr, &m_DebugMessenger
            ) == VK_SUCCESS,
