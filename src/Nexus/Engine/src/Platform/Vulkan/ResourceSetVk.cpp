...
namespace Nexus::Graphics
{
    ...
    )
    {
        ...
        // stores the number of dynamic offsets that need to be stored per descriptor
        // set
        ...
        for (const auto &[setIndex, dynamicOffsetCount] : setCounts)
        {
            offsetData[setIndex].resize(dynamicOffsetCount);
        }
    }

    ResourceSetVk::ResourceSetVk(PipelineHandle pipeline, GraphicsDeviceVk *device)
        : IResourceSet(pipeline), m_Pipeline(pipeline), m_Device(device)
    {
        ...
        const std::map<uint32_t, VkDescriptorSetLayout> &descriptorSetLayouts =
            vulkanPipeline->GetDescriptorSetLayouts();
