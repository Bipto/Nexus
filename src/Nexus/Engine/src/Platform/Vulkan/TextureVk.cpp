...
{
    TextureVk::TextureVk(const TextureDescription &spec, GraphicsDeviceVk *device)
        : ITexture(spec), m_GraphicsDevice(device)
    {
    ...
    TextureVk::~TextureVk()
    {
        ...
        for (const auto &[viewInfo, view] : m_ImageViews)
        {
            context.DestroyImageView(m_GraphicsDevice->GetVkDevice(), view, nullptr);
        }
    ...
    ) const
    {
        ...
        if (m_Description.CreateFlags & TextureCreateFlags_SparseBinding)
        {
            ...
            // this is most likely an image that was created with
            // VK_IMAGE_TILING_OPTIMAL and therefore does not have a row pitch
