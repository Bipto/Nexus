...
namespace Nexus::GL
{
    BoundingClientRect GetBoundingClientRect(const std::string &id)
    {
        ...
        emscripten::val rect =
            element.call<emscripten::val>("getBoundingClientRect");

        if (rect.isUndefined() || rect.isNull())
        {
            std::cout << "Could not get bounding client rect from element"
                      << std::endl;
            return {};
        }
    ...
    void Nexus::GL::ViewContextWebGL::Swap()
    {
        ...
        OffscreenContextWebGL *offscreenContext =
            (OffscreenContextWebGL *)m_Device->GetOffscreenContext();
        ...
        BoundingClientRect fullscreenRect =
            GetBoundingClientRect(offscreenCanvasName);
        ...
        glBlitFramebuffer(
            0, 0, textureWidth, textureHeight, x, y, x + textureWidth,
            y + textureHeight, GL_COLOR_BUFFER_BIT, GL_LINEAR
        );
    ...
    void Nexus::GL::ViewContextWebGL::CreateFramebuffer()
    {
        ...
        std::cout << "Creating framebuffer: [Width: " << framebufferSpec.Width
                  << ", Height: " << framebufferSpec.Height << "]" << std::endl;

        framebufferSpec.ColourAttachmentSpecification.Attachments = {
            Graphics::PixelFormat::R8_G8_B8_A8_UNorm
        };
        framebufferSpec.DepthAttachmentSpecification =
            Graphics::PixelFormat::D24_UNorm_S8_UInt;
