...
namespace Nexus::Graphics
{
    PhysicalDeviceEGL::PhysicalDeviceEGL(bool debug, EGLDisplay display)
    {
        ...
        std::string vendor = reinterpret_cast<const char *>(m_OffscreenContext->GetString(GL_VENDOR));
        std::string renderer = reinterpret_cast<const char *>(m_OffscreenContext->GetString(GL_RENDERER));
