...
#define GL_MIRROR_CLAMP_TO_EDGE_EXT -1
#define GL_BGRA_EXT -1
#define GL_R16 -1
#define GL_RG16 -1
#define GL_RGBA16_EXT -1
#define GL_COMPRESSED_RGBA -1
...
#define GL_COMPRESSED_RED_RGTC1 -1
#define GL_COMPRESSED_SIGNED_RED_RGTC1 -1
#define GL_COMPRESSED_RG_RGTC2 -1
#define GL_COMPRESSED_SIGNED_RG_RGTC2 -1
#define GL_COMPRESSED_RGBA_BPTC_UNORM -1
#define GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM -1
#define GL_DEPTH_COMPONENT32 -1
#define GL_MESH_SHADER_EXT -1
#define GL_TASK_SHADER_EXT -1
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT -1
#define GL_TEXTURE_LOD_BIAS_EXT -1
#define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT -1
#define GL_TEXTURE_LOD_BIAS_EXT -1
#define GL_TIMESTAMP -1
#define GL_MAX_IMAGE_SAMPLES -1
...
namespace Nexus::GL
{
    ...
    GLInternalTextureFormat GetGLInternalTextureFormat(const Graphics::TextureDescription &spec)
    {
        ...
        default:
            throw std::runtime_error("Failed to find a valid GLInternalTextureFormat");
        }
    }
    ...
    void ValidateFramebuffer(GLuint framebuffer, GL::IGLContext *context)
    {
        ...
        case GL_FRAMEBUFFER_UNDEFINED:
            throw std::runtime_error("The specified framebuffer is the default read or write framebuffer "
                                     "but the default framebuffer does not exist");
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            throw std::runtime_error("An attachment is incomplete");
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            throw std::runtime_error("This framebuffer does not have any attachments");
        ...
        case GL_FRAMEBUFFER_UNSUPPORTED:
            throw std::runtime_error("The framebuffer pixel format(s) are unsupported");
        case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
            throw std::runtime_error("The attachments have mismatching multisample levels");
        case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
            throw std::runtime_error("Framebuffer attachment is layered");
        }
    }

    void AttachTextureNonDSA(GLuint framebuffer, const Graphics::FramebufferTextureDescription &desc, bool isDepth,
                             uint32_t colourIndex, GL::IGLContext *context)
    {
        ...
        case GLInternalTextureFormat::Texture3D: {
            ...
            {
                context->FramebufferTextureLayer(GL_FRAMEBUFFER, attachmentType, textureHandle, desc.MipLevel,
                                                 desc.BaseArrayLayer);
            }
            else
            {
                context->FramebufferTextureMultiviewOVR(GL_FRAMEBUFFER, attachmentType, textureHandle, desc.MipLevel,
                                                        desc.BaseArrayLayer, desc.LayerCount);
            }
            break;
        }
        case GLInternalTextureFormat::Cubemap:
            context->FramebufferTexture2D(GL_FRAMEBUFFER, GL_TEXTURE_CUBE_MAP_POSITIVE_X + desc.BaseArrayLayer,
...
#error No OpenGL backend available
    ...
    void CopyBufferToTexture(const Graphics::CopyBufferToTextureCommand &command, GL::IGLContext *context)
    {
        ...
        if (command.BufferTextureCopy.TextureOffset.Z > 1)
        {
            ...
            NX_VALIDATE(textureDesc.Type == Graphics::TextureType::Texture3D ||
        ...
        NX_VALIDATE(context->IsTextureTypeSupported(texture->GetType(), texture->GetSampleCount()),
        ...
        case GL::GLInternalTextureFormat::Texture1D: {
            ...
            {
                context->TexSubImage1D(texture->GetHandle(), texture->GetTextureType(),
    ...
    void CopyTextureToBuffer(const Graphics::CopyTextureToBufferCommand &command, GL::IGLContext *context)
    {
        ...
        context->ReadPixels(command.TextureBufferCopy.TextureOffset.X, command.TextureBufferCopy.TextureOffset.Y,
    ...
    void CopyTextureToTexture(const Graphics::TextureCopyDescription &copyDesc, GL::IGLContext *context)
    {
        ...
        context->BlitFramebuffer(sourceFramebufferHandle, destFramebufferHandle, copyDesc.SourceOffset.X,
                                 ...
                                 copyDesc.DestinationOffset.Y + copyDesc.Extent.Height, GL_COLOR_BUFFER_BIT,
                                 GL_NEAREST);
