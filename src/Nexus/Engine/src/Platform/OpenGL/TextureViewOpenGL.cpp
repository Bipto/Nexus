...
namespace Nexus::Graphics
{
    bool IsFullTextureView(const TextureViewDescription &desc)
    {
        ...
        return textureDesc.Format == desc.Format && desc.Range.BaseMipLevel == 0 &&
               desc.Range.BaseArrayLayer == 0 &&
               desc.Range.LayerCount == textureDesc.DepthOrArrayLayers &&
               desc.Range.LevelCount == textureDesc.MipLevels;
    }
        ...
        : m_Description(desc), m_Device(device)
    {
        const TextureDescription &textureDesc =
            m_Description.TargetTexture->GetDescription();

        // if the texture view matches the full texture, we can use the texture
        // directly skipping the view
    ...
    void TextureViewOpenGL::Bind(uint32_t slot) const
    {
        if (m_TextureViewRequired)
        {
            ...
            context->Execute([&](const GladGLContext &gladContext) {
                const bool textureViewSupported =
                    gladContext.TextureViewEXT != nullptr;
                ...
                if (m_Handle)
                {
                    ...
                    else
                    {
                        const TextureOpenGL *texture =
                            m_EmulatedTextureView.AsDerived<const TextureOpenGL>();
                        texture->Bind(slot);
                    }
                }
            });
        ...
        {
            const TextureOpenGL *texture =
                m_Description.TargetTexture.AsDerived<const TextureOpenGL>();
            texture->Bind(slot);
        }
    ...
    void TextureViewOpenGL::CreateTextureView(const GladGLContext &context)
    {
        ...
        context.TextureViewEXT(
            m_Handle, m_ViewType, texture->GetHandle(), internalFormat,
    ...
    void TextureViewOpenGL::CreateEmulatedView(const GladGLContext &context)
    {
        ...
        const TextureDescription &originalDesc =
            m_Description.TargetTexture->GetDescription();
        ...
        const TextureOpenGL *emulatedTexture =
            m_EmulatedTextureView.AsDerived<const TextureOpenGL>();
        m_Handle = emulatedTexture->GetHandle();
    }

    void TextureViewOpenGL::UpdateEmulatedView(
        GL::IGLContext *context, const GladGLContext &gladContext
    ) const
    {
        const TextureOpenGL *source =
            m_Description.TargetTexture.AsDerived<const TextureOpenGL>();
             ...
             arrayLayer++)
        {
                 ...
                 mip++)
            {
                ...
                copyDesc.DestinationMipLevel =
                    mip - m_Description.Range.BaseMipLevel;
