...
namespace Nexus::Audio
{
	...
	class AudioDevice
	{
	  public:
		...
		NX_API virtual ~AudioDevice()
		{
		}

		/// @brief A method providing the currently used audio backend
		/// @return An AudioAPI enum representing the current backend
		virtual NX_API AudioAPI GetAPI() = 0;

		/// @brief A method that returns an AudioBuffer that is created from a .wav
		/// file
		/// @param filepath The path to the .wav file
		/// @return An audio buffer that can be used to create an AudioSource
		virtual NX_API Ref<AudioBuffer> CreateAudioBufferFromWavFile(const std::string &filepath) = 0;

		/// @brief A method that returns an AudioBuffer that is created from a .mp3
		/// file
		/// @param filepath The path to the .mp3 file
		/// @return An audio buffer that can be used to create an AudioSource
		virtual NX_API Ref<AudioBuffer> CreateAudioBufferFromMP3File(const std::string &filepath) = 0;
		...
		virtual NX_API Ref<AudioSource> CreateAudioSource(Ref<AudioBuffer> buffer) = 0;

		/// @brief A method that will play a sound effect from an AudioSource
		/// @param source The AudioSource to play the effect from
		virtual void NX_API PlaySource(Ref<AudioSource> source) = 0;
	};
}	 // namespace Nexus::Audio