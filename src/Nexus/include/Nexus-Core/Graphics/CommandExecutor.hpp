...
namespace Nexus::Graphics
{
	class NX_API CommandExecutor
	{
	  ...
	  private:
		...
		virtual void ExecuteCommand(DrawMeshDescription command, GraphicsDevice *device)							  = 0;
		virtual void ExecuteCommand(DrawMeshIndirectDescription command, GraphicsDevice *device)					  = 0;
		...
		virtual void ExecuteCommand(SetStencilReferenceCommand command, GraphicsDevice *device)						  = 0;
		virtual void ExecuteCommand(BuildAccelerationStructuresCommand command, GraphicsDevice *device)				  = 0;
		virtual void ExecuteCommand(AccelerationStructureCopyDescription command, GraphicsDevice *Device)			  = 0;
		virtual void ExecuteCommand(AccelerationStructureDeviceBufferCopyDescription command, GraphicsDevice *device) = 0;
		virtual void ExecuteCommand(DeviceBufferAccelerationStructureCopyDescription command, GraphicsDevice *device) = 0;
