...
namespace Nexus::Graphics
{
    bool Nexus::Graphics::FramebufferTextureSetDescription::ValidateSamples() const
    {
        ...
        // we need to retrieve the first valid texture from the framebuffer so that
        // we can retrieve it's sample count
        if (ColourAttachments.size() > 0)
        {
            testSampleCount = ColourAttachments[0]
                                  .ColourAttachment.TargetTexture->GetSampleCount();
        }
        // if there are no colour attachment, we try to get the depth attachment
        else if (DepthAttachment.has_value())
        {
            testSampleCount =
                DepthAttachment.value().TargetTexture->GetSampleCount();
        }
        ...
        {
            // the sample count does not match the first texture's so this set is
            // invalid
            if (colourAttachment.ColourAttachment.TargetTexture->GetSampleCount() !=
                testSampleCount)
            {
                return false;
            }
            ...
            if (colourAttachment.ResolveAttachment.has_value() &&
                colourAttachment.ResolveAttachment.value()
                        .TargetTexture->GetSampleCount() != 1)
            {
                return false;
            }
        }
        ...
        {
            // the depth textures sample count does not match the first texture's so
            // this set is invalid
    ...
    bool Nexus::Graphics::FramebufferTextureSetDescription::
        ValidateDimensions() const
    {
        ...
        // we need to retrieve the first valid texture from the framebuffer so that
        // we can retrieve it's width and height
        if (ColourAttachments.size() > 0)
        {
            TextureHandle colourAttachment =
                ColourAttachments[0].ColourAttachment.TargetTexture;
        ...
        for (const auto &colourAttachment : ColourAttachments)
        {
            ...
            // the dimensions of this texture does not match the first texture's so
            // this set is invalid
            if (colourTexture->GetWidth() != testWidth ||
                colourTexture->GetHeight() != testHeight)
            {
                return false;
            }
            ...
            if (colourAttachment.ResolveAttachment.has_value())
            {
                ...
                {
                    // the dimensions of this texture does not match the first
                    // texture's so this set is invalid
                    if (resolveTexture->GetWidth() != testWidth ||
                        resolveTexture->GetHeight() != testHeight)
                    {
                        return false;
                    }
                }
        ...
        if (DepthAttachment.has_value())
        {
            ...
            // the depth textures dimensions does not match the first texture's so
            // this set is invalid
            if (texture->GetWidth() != testWidth ||
                texture->GetHeight() != testHeight)
            {
                return false;
            }
        }
    ...
    bool Nexus::Graphics::FramebufferTextureSetDescription::
        ValidateHasTexture() const
    {
        // checks whether there are any colour attachments or a depth attachment in
        // this framebuffer
    ...
    bool FramebufferTextureSetDescription::ValidateUsageFlags() const
    {
        ...
        for (const auto &colourAttachment : ColourAttachments)
        {
            // a texture that is being in a framebuffer as a colour attachment must
            // have the ColourAttachment usage flag set
            TextureHandle colourTexture =
                colourAttachment.ColourAttachment.TargetTexture;
            if (!(colourTexture->GetUsage() &
                  Graphics::TextureUsage_ColourAttachment))
            {
                return false;
            }
            ...
            if (colourAttachment.ResolveAttachment.has_value())
            {
                ...
                if (resolveTexture.IsValid())
                {
                    // a texture that is being in a framebuffer as a resolve
                    // attachment must have the ColourAttachment usage flag set
                    if (!(resolveTexture->GetUsage() &
                          Graphics::TextureUsage_ColourAttachment))
                    {
                        return false;
                    }
        ...
        if (DepthAttachment.has_value())
        {
            // a texture that is being in a framebuffer as a depth/stencil attachment
            // must have the DepthStencilAttachment usage flag set
            TextureHandle texture = DepthAttachment.value().TargetTexture;
            if (!(texture->GetUsage() &
    ...
    uint32_t FramebufferTextureSetDescription::GetSampleCount() const
    {
        ...
        // otherwise we try to retrieve the sample count from the depth/stencil
        // attachment
        if (DepthAttachment.has_value())
        {
            TextureHandle texture = DepthAttachment.value().TargetTexture;
    ...
    std::pair<uint32_t, uint32_t> FramebufferTextureSetDescription::GetSize() const
    {
        // we first try to retrieve the size from a colour attachment
        // technically, this doesn't need to be a loop, but this is simpler
        for (const auto &colourAttachment : ColourAttachments)
        {
            TextureHandle texture = colourAttachment.ColourAttachment.TargetTexture;
