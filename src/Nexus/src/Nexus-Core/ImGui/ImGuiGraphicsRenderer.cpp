...
static void ImGui_ImplNexus_SetPlatformImeData(ImGuiViewport *vp,
                                               ImGuiPlatformImeData *data) {
  Nexus::ImGuiUtils::ImGuiWindowInfo *info =
      (Nexus::ImGuiUtils::ImGuiWindowInfo *)vp->PlatformUserData;
  if (data->WantVisible) {
    Nexus::Graphics::Rectangle<int> rect = {(int)data->InputPos.x,
                                            (int)data->InputPos.y, 1,
...
ImGuiGraphicsRenderer::ImGuiGraphicsRenderer(Nexus::Application *app)
  ...
  auto vertexModule = m_GraphicsDevice->CreateShaderModuleFromSpirvSource(
      vertexSource, "ImGui.vert.glsl", Nexus::Graphics::ShaderStage::Vertex);
  auto fragmentModule = m_GraphicsDevice->CreateShaderModuleFromSpirvSource(
      fragmentSource, "ImGui.frag.glsl",
  ...
  pipelineDesc.ColourTargetSampleCount = m_GraphicsDevice->GetPrimaryWindow()
                                             ->GetSwapchain()
                                             ->GetSpecification()
                                             .Samples;
  ...
  pipelineDesc.BlendStateDesc.SourceColourBlend =
      Nexus::Graphics::BlendFactor::SourceAlpha;
  pipelineDesc.BlendStateDesc.DestinationColourBlend =
      Nexus::Graphics::BlendFactor::OneMinusSourceAlpha;
  pipelineDesc.BlendStateDesc.ColorBlendFunction =
      Nexus::Graphics::BlendEquation::Add;
  pipelineDesc.BlendStateDesc.SourceAlphaBlend =
      Nexus::Graphics::BlendFactor::One;
  pipelineDesc.BlendStateDesc.DestinationAlphaBlend =
      Nexus::Graphics::BlendFactor::OneMinusSourceAlpha;
  pipelineDesc.BlendStateDesc.AlphaBlendFunction =
      Nexus::Graphics::BlendEquation::Add;
  ...
  pipelineDesc.DepthStencilDesc.DepthComparisonFunction =
      Nexus::Graphics::ComparisonFunction::Always;
  ...
  pipelineDesc.DepthStencilDesc.StencilComparisonFunction =
      Nexus::Graphics::ComparisonFunction::Always;
  ...
  m_UniformBuffer =
      m_GraphicsDevice->CreateUniformBuffer(uniformBufferDesc, nullptr);
  ...
  platformIo.Platform_CreateWindow = [](ImGuiViewport *vp) -> void {
    ...
    Nexus::Graphics::SwapchainSpecification swapchainSpec =
        app->GetPrimaryWindow()->GetSwapchain()->GetSpecification();

    Nexus::Window *window =
        app->CreateApplicationWindow(windowSpec, swapchainSpec);
...
void ImGuiGraphicsRenderer::AfterLayout() {
  ...
  if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
    ...
    for (int i = platform_io.Viewports.Size - 1; i >= 1; i--) {
      if ((platform_io.Viewports[i]->Flags & ImGuiViewportFlags_IsMinimized) ==
          0) {
...
void ImGuiGraphicsRenderer::SetupInput() {
  ...
  m_Keys.push_back(io.KeyMap[(int)ImGuiKey_RightArrow] =
                       (int)KeyCode::KeyRight);
...
void ImGuiGraphicsRenderer::UpdateInput() {
  ...
  io.KeyShift = keyboard.IsKeyHeld(KeyCode::LeftShift) ||
                keyboard.IsKeyHeld(KeyCode::RightShift);
  io.KeyCtrl = keyboard.IsKeyHeld(KeyCode::LeftControl) ||
               keyboard.IsKeyHeld(KeyCode::RightControl);
  io.KeyAlt = keyboard.IsKeyHeld(KeyCode::LeftAlt) ||
              keyboard.IsKeyHeld(KeyCode::RightAlt);
  io.KeySuper = keyboard.IsKeyHeld(KeyCode::LeftWin) ||
                keyboard.IsKeyHeld(KeyCode::RightWin);

  io.DisplaySize = {(float)window->GetWindowSize().X,
                    (float)window->GetWindowSize().Y};
  io.DisplayFramebufferScale = {1, 1};

  if (ImGui::GetIO().ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
    io.MousePos = {(float)Mouse::GetGlobalMousePosition().X,
                   (float)Mouse::GetGlobalMousePosition().Y};
  } else {
    io.MousePos = {(float)mouse.GetMousePosition().X,
                   (float)mouse.GetMousePosition().Y};
  }
...
void ImGuiGraphicsRenderer::UpdateBuffers(ImDrawData *drawData) {
  ...
  if (drawData->TotalIdxCount > m_IndexBufferCount) {
    ...
    desc.Usage = Nexus::Graphics::BufferUsage::Dynamic;
    m_IndexBuffer = m_GraphicsDevice->CreateIndexBuffer(
        desc, nullptr, Nexus::Graphics::IndexBufferFormat::UInt16);
  ...
  // update vertex buffer
  {
    ...
    for (int i = 0; i < drawData->CmdListsCount; i++) {
      ...
      m_VertexBuffer->SetData(cmdList->VtxBuffer.Data,
                              cmdList->VtxBuffer.size_in_bytes(), offset);
  ...
  // update index buffer
  {
    ...
    for (int i = 0; i < drawData->CmdListsCount; i++) {
      const ImDrawList *cmdList = drawData->CmdLists[i];
      m_IndexBuffer->SetData(cmdList->IdxBuffer.Data,
                             cmdList->IdxBuffer.size_in_bytes(), offset);
...
void ImGuiGraphicsRenderer::RenderCommandLists(ImDrawData *drawData) {
  ...
  m_CommandList->SetRenderTarget(
      Nexus::Graphics::RenderTarget(info->Window->GetSwapchain()));
  ...
  auto &io = ImGui::GetIO();
  glm::mat4 mvp =
      glm::ortho<float>(pos.x, pos.x + drawData->DisplaySize.x,
                        pos.y + drawData->DisplaySize.y, pos.y, -1.f, 1.0f);
  ...
  for (int n = 0; n < drawData->CmdListsCount; n++) {
    ...
    for (int cmdi = 0; cmdi < cmdList->CmdBuffer.Size; cmdi++) {
      ...
      if (drawCmd.ElemCount > 0) {
        ...
        m_CommandList->DrawIndexed(drawCmd.ElemCount,
