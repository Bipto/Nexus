...
public:
  ...
  void SetPoints(const std::vector<Point2D<T>> &points) { m_Points = points; }

  const std::vector<Point2D<T>> &GetPoints() const { return m_Points; }

  const size_t GetNumberOfPoints() const { return m_Points.size(); }
