...
std::map<std::string, uint32_t> ResourceSet::RemapToLinearBindings(
    const std::vector<ResourceBinding> &resources) {
  uint32_t resourceIndex = 0;
  std::map<std::string, uint32_t> bindings;

  for (const auto &resource : resources) {
    uint32_t remappedBinding =
        GetLinearDescriptorSlot(resource.Set, resource.Binding);
...
ResourceSet::GetBoundUniformBuffers() const {
  return m_BoundUniformBuffers;
}

const std::map<std::string, CombinedImageSampler> &
ResourceSet::GetBoundCombinedImageSamplers() const {
