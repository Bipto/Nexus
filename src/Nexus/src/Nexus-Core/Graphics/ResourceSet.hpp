...
enum class ResourceType { CombinedImageSampler, UniformBuffer };
...
struct ResourceSetSpecification {
  ...
  ResourceSetSpecification &operator+(const ResourceSetSpecification &other) {
    SampledImages.insert(SampledImages.end(), other.SampledImages.begin(),
                         other.SampledImages.end());
    UniformBuffers.insert(UniformBuffers.end(), UniformBuffers.begin(),
                          UniformBuffers.end());
    return *this;
  }
  ...
  ResourceSetSpecification &operator+=(const ResourceSetSpecification &other) {
    SampledImages.insert(SampledImages.end(), other.SampledImages.begin(),
                         other.SampledImages.end());
    UniformBuffers.insert(UniformBuffers.end(), other.UniformBuffers.begin(),
                          other.UniformBuffers.end());
    return *this;
  }
};
...
public:
  ...
  RemapToLinearBindings(const std::vector<ResourceBinding> &resources);

  const std::map<std::string, WeakRef<UniformBuffer>> &
  GetBoundUniformBuffers() const;
  const std::map<std::string, CombinedImageSampler> &
  GetBoundCombinedImageSamplers() const;
