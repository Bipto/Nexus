...
protected:
  bool ValidateForGraphicsCall(std::optional<Ref<Pipeline>> pipeline,
                               std::optional<RenderTarget> renderTarget);
  bool ValidateForClearColour(std::optional<RenderTarget> target,
                              uint32_t colourIndex);
  bool ValidateForClearDepth(std::optional<RenderTarget> target);
  bool ValidateForSetViewport(std::optional<RenderTarget> target,
                              const Viewport &viewport);
  bool ValidateForSetScissor(std::optional<RenderTarget> target,
                             const Scissor &scissor);
  bool ValidateForResolveToSwapchain(
