...
void MipmapGenerator::GenerateMips(Ref<Texture2D> texture, uint32_t mipCount) {
  ...
  Ref<ShaderModule> vertexModule = m_Device->CreateShaderModuleFromSpirvSource(
      c_MipmapVertexSource, "Mipmap-Gen.vert",
      Nexus::Graphics::ShaderStage::Vertex);
  Ref<ShaderModule> fragmentModule =
      m_Device->CreateShaderModuleFromSpirvSource(
          c_MipmapFragmentSource, "Mipmap-Gen.frag",
  ...
  for (uint32_t i = 1; i <= mipCount; i++) {
    ...
    // create mip
    {
      ...
      framebufferSpec.ColorAttachmentSpecification = {
          texture->GetSpecification().Format};
      ...
      Ref<Framebuffer> framebuffer =
          m_Device->CreateFramebuffer(framebufferSpec);
      ...
      m_CommandList->SetRenderTarget(
          Nexus::Graphics::RenderTarget(framebuffer));
      ...
      uint32_t framebufferHeight =
          framebufferTexture->GetSpecification().Height;

      std::vector<std::byte> pixels = framebufferTexture->GetData(
          0, 0, 0, framebufferWidth, framebufferHeight);
      MipData mipData(pixels, framebufferWidth, framebufferHeight);
      texture->SetData(mipData.GetData(), i, 0, 0, framebufferWidth,
                       framebufferHeight);
...
uint32_t MipmapGenerator::GetMaximumNumberOfMips(uint32_t width,
                                                 uint32_t height) {
  return std::floor(std::log2(std::max(width, height)));
}

MipData::MipData(const std::vector<std::byte> &pixels, uint32_t width,
                 uint32_t height)
    : m_Pixels(pixels), m_Width(width), m_Height(height) {}

uint32_t MipData::GetWidth() const { return m_Width; }

uint32_t MipData::GetHeight() const { return m_Height; }

const void *MipData::GetData() const { return m_Pixels.data(); }
} // namespace Nexus::Graphics