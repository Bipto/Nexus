...
HdriProcessor::HdriProcessor(const std::string &filepath,
...
Ref<Cubemap> HdriProcessor::Generate(uint32_t size) {
  ...
  pipelineDescription.VertexModule =
      m_Device->CreateShaderModuleFromSpirvSource(
          HdriVertexShaderSource, "hdri.vert.glsl",
          Nexus::Graphics::ShaderStage::Vertex);
  pipelineDescription.FragmentModule =
      m_Device->CreateShaderModuleFromSpirvSource(
          HdriFragmentShaderSource, "hdri.frag.glsl",
          Nexus::Graphics::ShaderStage::Fragment);
  pipelineDescription.ResourceSetSpec.UniformBuffers = {{"Camera", 0, 0}};
  pipelineDescription.ResourceSetSpec.SampledImages = {
      {"equirectangularMap", 1, 0}};

  pipelineDescription.ColourFormats[0] =
      framebufferSpec.ColorAttachmentSpecification.Attachments[0].TextureFormat;
  pipelineDescription.ColourTargetCount = 1;
  pipelineDescription.DepthFormat =
      framebufferSpec.DepthAttachmentSpecification.DepthFormat;
  ...
  Ref<UniformBuffer> uniformBuffer =
      m_Device->CreateUniformBuffer(cameraUniformBufferDesc, nullptr);
  ...
  for (uint32_t i = 0; i < 6; i++) {
    ...
    resourceSet->WriteCombinedImageSampler(m_HdriImage, sampler,
                                           "equirectangularMap");
    ...
    auto indexCount =
        cube->GetIndexBuffer()->GetDescription().Size / sizeof(unsigned int);
