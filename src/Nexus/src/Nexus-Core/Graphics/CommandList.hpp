...
typedef std::variant<
    SetVertexBufferCommand, WeakRef<IndexBuffer>, WeakRef<Pipeline>,
    DrawElementCommand, DrawIndexedCommand, DrawInstancedCommand,
    DrawInstancedIndexedCommand, Ref<ResourceSet>, ClearColorTargetCommand,
    ClearDepthStencilTargetCommand, RenderTarget, Viewport, Scissor,
    ResolveSamplesToSwapchainCommand, StartTimingQueryCommand,
    StopTimingQueryCommand, SetStencilRefCommand, SetDepthBoundsCommand,
...
public:
  ...
  virtual ~CommandList() { m_Commands.clear(); }
  ...
  void DrawInstanced(uint32_t vertexCount, uint32_t instanceCount,
                     uint32_t vertexStart, uint32_t instanceStart);
  ...
  void ResolveFramebuffer(Ref<Framebuffer> source, uint32_t sourceIndex,
                          Swapchain *target);
