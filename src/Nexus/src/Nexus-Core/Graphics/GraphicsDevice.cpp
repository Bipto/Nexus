...
Ref<ShaderModule>
GraphicsDevice::CreateShaderModuleFromSpirvFile(const std::string &filepath,
                                                ShaderStage stage) {
...
Ref<ShaderModule> GraphicsDevice::CreateShaderModuleFromSpirvSource(
    const std::string &source, const std::string &name, ShaderStage stage) {
...
Window *GraphicsDevice::GetPrimaryWindow() { return m_Window; }

void GraphicsDevice::ImmediateSubmit(
    std::function<void(Ref<CommandList>)> &&function) {
...
Ref<Texture2D> GraphicsDevice::CreateTexture2D(const char *filepath,
                                               bool generateMips) {
  ...
  unsigned char *data =
      stbi_load(filepath, &width, &height, &receivedChannels, desiredChannels);
  spec.Width = (uint32_t)width;
  spec.Height = (uint32_t)height;

  if (generateMips) {
    uint32_t mipCount =
        Nexus::Graphics::MipmapGenerator::GetMaximumNumberOfMips(spec.Width,
                                                                 spec.Height);
...
Ref<Texture2D> GraphicsDevice::CreateTexture2D(const std::string &filepath,
                                               bool generateMips) {
