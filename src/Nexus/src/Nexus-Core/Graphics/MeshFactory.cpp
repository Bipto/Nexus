...
Ref<Mesh> MeshFactory::CreateCube() {
  ...
  auto vertexBuffer =
      m_Device->CreateVertexBuffer(vertexBufferDesc, fullVertices.data());
  ...
  auto indexBuffer =
      m_Device->CreateIndexBuffer(indexBufferDesc, indices.data());
...
Ref<Mesh> MeshFactory::CreateSprite() {
  ...
  vertexBufferDesc.Usage = Nexus::Graphics::BufferUsage::Static;
  auto vertexBuffer =
      m_Device->CreateVertexBuffer(vertexBufferDesc, vertices.data());
...
Ref<Mesh> ProcessMesh(aiMesh *mesh, const aiScene *scene,
  ...
  vertexBufferDesc.Usage = Nexus::Graphics::BufferUsage::Static;
  auto vertexBuffer =
      device->CreateVertexBuffer(vertexBufferDesc, vertices.data());
...
void ProcessNode(aiNode *node, const aiScene *scene,
                 std::vector<Ref<Mesh>> &meshes, GraphicsDevice *device) {
...
Ref<Model> MeshFactory::CreateFrom3DModelFile(const std::string &filepath) {
  ...
  auto flags = aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices |
