...
enum class RenderTargetType { Swapchain, Framebuffer };
...
public:
  ...
  explicit RenderTarget(Swapchain *swapchain)
      : m_Target(swapchain), m_RenderTargetType(RenderTargetType::Swapchain) {}

  explicit RenderTarget(Ref<Framebuffer> framebuffer)
      : m_Target(framebuffer),
        m_RenderTargetType(RenderTargetType::Framebuffer) {}

  template <typename T> T GetData() { return std::get<T>(m_Target); }

  template <typename T> T *GetDataIf() { return std::get_if<T>(&m_Target); }

  RenderTargetType GetType() { return m_RenderTargetType; }
  ...
  bool HasDepthAttachment() {
    if (m_RenderTargetType == RenderTargetType::Swapchain) {
      ...
      auto framebuffer = GetData<Ref<Framebuffer>>();
      const auto &framebufferSpec = framebuffer->GetFramebufferSpecification();
      return framebufferSpec.DepthAttachmentSpecification.DepthFormat !=
             PixelFormat::None;
    }
  }
