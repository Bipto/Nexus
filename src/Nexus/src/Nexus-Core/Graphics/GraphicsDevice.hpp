...
public:
  ...
  GraphicsDevice(const GraphicsDeviceSpecification &createInfo, Window *window,
  ...
  CreateIndexBuffer(const BufferDescription &description, const void *data,
  ...
  CreateCommandList(const CommandListSpecification &spec = {}) = 0;
  ...
  CreateTexture2D(const Texture2DSpecification &spec) = 0;
  ...
  /// @param filepath The filepath to load the image from
  /// @return A pointer to a texture
  Ref<Texture2D> CreateTexture2D(const std::string &filepath,
                                 bool generateMips);

  virtual Ref<Cubemap> CreateCubemap(const CubemapSpecification &spec) = 0;

  virtual Ref<Framebuffer>
  CreateFramebuffer(const FramebufferSpecification &spec) = 0;
  ...
  CreateResourceSet(const ResourceSetSpecification &spec) = 0;
  ...
  Ref<ShaderModule> CreateShaderModuleFromSpirvFile(const std::string &filepath,
                                                    ShaderStage stage);

  Ref<ShaderModule> CreateShaderModuleFromSpirvSource(const std::string &source,
...
private:
  ...
  CreateShaderModule(const ShaderModuleSpecification &moduleSpec,
                     const ResourceSetSpecification &resources) = 0;
