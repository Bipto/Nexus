...
public:
  ...
  RoundedRectangle(const Point2D<float> &position, const Point2D<float> &size,
                   float radiusTL, float radiusTR, float radiusBL,
                   float radiusBR)
      : m_X(position.X), m_Y(position.Y), m_Width(size.X), m_Height(size.Y),
        m_RadiusTL(radiusTL), m_RadiusTR(radiusTR), m_RadiusBL(radiusBL),
        m_RadiusBR(radiusBR) {}

  RoundedRectangle(float x, float y, float width, float height, float radiusTL,
  ...
  RoundedRectangle(const Rectangle<float> &rect, float radiusTL, float radiusTR,
  ...
  void SetPointsPerCorner(size_t points) { m_PointsPerCorner = points; }

  const size_t GetPointsPerCorner() const { return m_PointsPerCorner; }
  ...
  void SetRadiusTopLeft(float radius) { m_RadiusTL = radius; }

  void SetRadiusTopRight(float radius) { m_RadiusTR = radius; }

  void SetRadiusBottomLeft(float radius) { m_RadiusBL = radius; }

  void SetRadiusBottomRight(float radius) { m_RadiusBR = radius; }

  const float GetLeft() const { return m_X; }

  const float GetRight() const { return m_X + m_Width; }

  const float GetTop() const { return m_Y; }

  const float GetBottom() const { return m_Y + m_Height; }

  const float GetHeight() const { return m_Height; }

  const float GetWidth() const { return m_Width; }

  const float GetRadiusTopLeft() const { return m_RadiusTL; }

  const float GetRadiusTopRight() const { return m_RadiusTR; }

  const float GetRadiusBottomLeft() const { return m_RadiusBL; }

  const float GetRadiusBottomRight() const { return m_RadiusBR; }

  glm::vec2 GetTopLeft() const { return {GetLeft(), GetTop()}; }

  glm::vec2 GetTopRight() const { return {GetRight(), GetTop()}; }

  glm::vec2 GetBottomLeft() const { return {GetLeft(), GetBottom()}; }

  glm::vec2 GetBottomRight() const { return {GetRight(), GetBottom()}; }
  ...
  const void Deconstruct(float *x, float *y, float *width, float *height,
                         float *radiusTL, float *radiusTR, float *radiusBL,
                         float *radiusBR, float *pointsPerCorner) const {
    if (x) {
      *x = m_X;
    }
  ...
  std::vector<glm::vec2> CreateOutline() const {
    const glm::vec2 tr = {GetRight() - GetRadiusTopRight(),
                          GetTop() + GetRadiusTopRight()};

    const glm::vec2 tl = {GetLeft() + GetRadiusTopLeft(),
                          GetTop() + GetRadiusTopLeft()};

    const glm::vec2 bl = {GetLeft() + GetRadiusBottomLeft(),
                          GetBottom() - GetRadiusBottomLeft()};

    const glm::vec2 br = {GetRight() - GetRadiusBottomRight(),
...
private:
  std::vector<glm::vec2> CreateCircleRegion(const glm::vec2 &position,
