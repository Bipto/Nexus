...
spv::ExecutionModel
GetShaderExecutionModel(Nexus::Graphics::ShaderStage stage) {
  switch (stage) {
...
void ToLinearResourceSet(spirv_cross::Compiler &compiler,
  ...
  for (const auto &uniformBuffer : resources.uniform_buffers) {
    ...
    if (language == ShaderLanguage::GLSL ||
        language == ShaderLanguage::GLSLES) {
...
void CreateResourceSetSpecification(const spirv_cross::Compiler &compiler,
...
ShaderDataType
GetShaderDataTypeFromSpirvCrossType(const spirv_cross::Compiler &compiler,
...
void GetShaderInfo(const spirv_cross::Compiler &compiler,
...
CompilationResult
ShaderGenerator::Generate(const std::string &source,
  ...
  shaderc::CompilationResult result =
      compiler.CompileGlslToSpv(source, shaderType, options.ShaderName.c_str());
  ...
  case ShaderLanguage::HLSL: {
    ...
    compiler.rename_entry_point("main", name.c_str(),
                                GetShaderExecutionModel(options.Stage));
  ...
  if (options.OutputFormat == ShaderLanguage::SPIRV) {
    ...
    shaderc::CompilationResult result = compiler.CompileGlslToSpv(
        output.Source, shaderType, options.ShaderName.c_str());
