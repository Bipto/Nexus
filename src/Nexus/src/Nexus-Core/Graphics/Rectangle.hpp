...
template <typename T> struct Rectangle
{
  public:
    ...
    Rectangle(const Point2D<T> &position, const Point2D<T> &size) : m_X(position.X), m_Y(position.Y), m_Width(size.X), m_Height(size.Y)
    {
    }
    ...
    {
        return other.X >= this->m_X && other.X < this->m_X + this->m_Width && other.Y >= this->m_Y && other.Y < this->m_Y + this->m_Height;
    }

    const bool Contains(const Rectangle<T> &other) const
    {
        return this->GetRight() <= other.GetLeft() && other.GetRight() <= this->GetRight() && this->GetTop() <= other.GetTop() && other.GetBottom() <= this->GetBottom();
    }

    const bool Intersects(const Rectangle &other) const
    {
        return other.GetRight() < this->GetRight() && this->GetLeft() < other.GetRight() && other.GetTop() < this->GetBottom() && this->GetTop() < other.GetBottom();
    }
