...
void LoadCharacter(char character, FT_Face &face,
                   Nexus::Graphics::FontData &data,
                   std::map<char, Nexus::Graphics::Character> &characters,
                   uint32_t xPos, uint32_t yPos, uint32_t textureWidth,
...
Nexus::Point2D<uint32_t>
FindLargestGlyphSize(const FT_Face &face,
...
Font::Font(const std::string &filepath, uint32_t size,
  ...
  for (const auto &range : m_CharacterRanges) {
    for (int character = range.Begin; character < range.End; character++) {
      LoadCharacter((char)character, face, pixels, m_Characters, xPos, yPos,
  ...
  m_Texture->SetData(pixels.GetPixels().data(), 0, 0, 0, pixels.GetWidth(),
                     pixels.GetHeight());
...
Nexus::Ref<Nexus::Graphics::Texture2D> Font::GetTexture() { return m_Texture; }

const Character &Font::GetCharacter(char character) {
  return m_Characters[character];
}

uint32_t Font::GetSize() const { return m_FontSize; }

Nexus::Point2D<float> Font::MeasureString(const std::string &text,
                                          uint32_t size) {
...
const uint32_t Font::GetLineHeight() const { return m_LineSpacing; }

const Point2D<uint32_t> Font::GetMaxCharacterSize() const {
  return m_MaxCharacterSize;
}

const FontType Font::GetFontType() const { return m_Type; }
} // namespace Nexus::Graphics