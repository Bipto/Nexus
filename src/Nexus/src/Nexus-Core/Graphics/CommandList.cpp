...
CommandList::CommandList(const CommandListSpecification &spec)
...
void CommandList::SetVertexBuffer(Ref<VertexBuffer> vertexBuffer,
                                  uint32_t slot) {
  if (!m_Started) {
...
void CommandList::DrawIndexed(uint32_t count, uint32_t indexStart,
...
void CommandList::ClearColorTarget(uint32_t index,
...
void CommandList::ResolveFramebuffer(Ref<Framebuffer> source,
                                     uint32_t sourceIndex, Swapchain *target) {
  if (!m_Started) {
    NX_ERROR("Attempting to record a command into a CommandList without "
             "calling Begin()");
    return;
