...
public:
  ...
  const Point2D<T> &GetPosition() const { return m_Position; }

  T GetRadius() const { return m_Radius; }

  void SetPosition(const Point2D<T> &position) { m_Position = position; }
  ...
  template <typename Other> const Circle To() const {
