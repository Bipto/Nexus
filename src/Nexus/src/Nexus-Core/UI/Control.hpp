...
namespace Nexus::UI
{
	...
	struct Padding
	{
		...
		explicit Padding(float padding) : Left(padding), Right(padding), Top(padding), Bottom(padding)
		{
		}
		...
		explicit Padding(float vertical, float horizontal) : Left(vertical), Right(vertical), Top(horizontal), Bottom(horizontal)
		{
		}
	};
