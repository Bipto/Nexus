...
namespace Nexus::UI {
Canvas::Canvas(Graphics::GraphicsDevice *device,
               Graphics::Swapchain *swapchain) {
  m_BatchRenderer = std::make_unique<Nexus::Graphics::BatchRenderer>(
      device, Nexus::Graphics::RenderTarget{swapchain});
...
void Canvas::SetSize(const Point2D<uint32_t> &size) { m_Size = size; }

void Canvas::SetBackgroundColour(const glm::vec4 &color) {
  m_BackgroundColour = color;
}

const Point2D<uint32_t> &Canvas::GetPosition() const { return m_Position; }

const Point2D<uint32_t> &Canvas::GetSize() const { return m_Size; }
...
void Canvas::Render() const {
  ...
  Nexus::Graphics::Rectangle<float> rect((float)m_Position.X,
                                         (float)m_Position.Y, (float)m_Size.X,
                                         (float)m_Size.Y);
...
Nexus::Window *Canvas::GetWindow() const { return m_Window; }
} // namespace Nexus::UI