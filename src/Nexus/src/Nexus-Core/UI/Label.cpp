...
namespace Nexus::UI
{
	...
	void Nexus::UI::Label::OnRender(Nexus::Graphics::BatchRenderer *renderer)
	{
		...
		Nexus::Graphics::Rectangle<float> rect = GetControlBounds();
		Nexus::Graphics::RoundedRectangle rrect({rect.GetLeft() + m_BorderThickness,
		...
		{
			Nexus::Graphics::RoundedRectangle border(rrect.GetLeft() + m_BorderThickness,
													 rrect.GetTop() + m_BorderThickness,
