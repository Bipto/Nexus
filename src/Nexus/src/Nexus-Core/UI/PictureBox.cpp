...
void PictureBox::OnRender(Nexus::Graphics::BatchRenderer *renderer) {
  ...
  Nexus::Graphics::Polygon outlinePoly =
      Nexus::Utils::GeneratePolygon(outlinePoints);
  ...
  if (m_Texture) {
    Nexus::Graphics::RoundedRectangle textureBoundsWidthPadding(
        rect.GetLeft() + m_Padding.Left, rect.GetTop() + m_Padding.Top,
        rect.GetWidth() - m_Padding.Left - m_Padding.Right,
        rect.GetHeight() - m_Padding.Top - m_Padding.Bottom, m_CornerRounding,
        m_CornerRounding, m_CornerRounding, m_CornerRounding);

    std::vector<glm::vec2> textureBoundsOutlinePoints =
        textureBoundsWidthPadding.CreateOutline();

    if (m_Parent) {
      Graphics::RoundedRectangle parentRect = m_Parent->GetRoundedRectangle();
      textureBoundsOutlinePoints =
          textureBoundsWidthPadding.ClipAgainst(parentRect);
    }

    Nexus::Graphics::Polygon textureBoundsPoly =
        Nexus::Utils::GeneratePolygon(textureBoundsOutlinePoints);
    renderer->DrawPolygonFill(textureBoundsPoly, {1.0f, 1.0f, 1.0f, 1.0f},
                              m_Texture, 1.0f);
  }

  /* if (m_BorderThickness > 0.0f)
  {
      renderer->DrawRoundedRectangle(outlineRect, m_BorderColour,
  m_BorderThickness);
  } */
...
void PictureBox::OnAutoSize() {
  if (m_Texture) {
    const Point2D<uint32_t> textureSize = {
        m_Texture->GetSpecification().Width,
        m_Texture->GetSpecification().Height};
