...
const Graphics::Rectangle<float> Control::GetControlBounds() const
{
    ...
    return Graphics::Rectangle<float>(pos.X, pos.Y, m_Size.X + m_Padding.Left + m_Padding.Right, m_Size.Y + m_Padding.Top + m_Padding.Bottom);
}
...
const std::vector<glm::vec2> Control::GetDrawableArea() const
{
    ...
    Nexus::Graphics::RoundedRectangle rrect(screenSpacePosition.X, screenSpacePosition.Y, m_Size.X, m_Size.Y, m_CornerRounding, m_CornerRounding, m_CornerRounding,
                                            m_CornerRounding);
