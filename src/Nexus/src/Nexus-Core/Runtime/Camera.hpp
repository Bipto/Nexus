...
public:
  FirstPersonCamera(int width = 1280, int height = 720,
  ...
  glm::mat4 GetViewProjection() const { return GetView() * m_Projection; }
...
private:
  void Move(Time time) {
    ...
    if (Input::IsGamepadConnected()) {
      m_Position -= speed * m_Front *
                    (Input::GetGamepadAxisLeft(0).Y *
                     (Input::GetGamepadRightTrigger(0) + 1));
      m_Position += glm::normalize(glm::cross(m_Front, m_Up)) *
                    (Input::GetGamepadAxisLeft(0).X / 50.0f *
                     (Input::GetGamepadRightTrigger(0) + 1));
    }
  }
    ...
    if (aspectRatio > 0.0f) {
      ...
      if (m_ProjectionType == ProjectionType::Orthographic) {
        m_Projection = glm::ortho<float>(
            ...
            (-m_Height / 2) / m_Zoom, (m_Height / 2) / m_Zoom, 0.1f, 1000.0f);
      }
      // perspective
      else {
        m_Projection =
            glm::perspectiveFov<float>(glm::radians(m_Zoom), (float)m_Width,
