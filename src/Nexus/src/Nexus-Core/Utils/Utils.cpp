...
glm::vec4 ColorFromRGBA(float r, float g, float b, float a) {
  return glm::vec4(1.0f / 255.0f * r, 1.0f / 255.0f * g, 1.0f / 255.0f * b,
                   1.0f / 255.0f * a);
}
...
void Clip(std::vector<glm::vec2> &points, float x1, float y1, float x2,
          float y2) {
...
std::vector<glm::vec2>
SutherlandHodgman(const std::vector<glm::vec2> &subjectPolygon,
                  const std::vector<glm::vec2> &clipPolygon) {
  assert(subjectPolygon.size() >= 3 &&
         "The subject polygon must have at least 3 points");
  assert(clipPolygon.size() >= 3 &&
...
bool Triangulate(const std::vector<glm::vec2> &polygon,
...
std::vector<Nexus::Graphics::Triangle2D>
GenerateGeometry(const std::vector<glm::vec2> &polygon,
...
Nexus::Graphics::Polygon
GeneratePolygon(const std::vector<glm::vec2> &polygon) {
...
std::vector<glm::vec2>
ReverseWindingOrder(const std::vector<glm::vec2> &vertices) {
