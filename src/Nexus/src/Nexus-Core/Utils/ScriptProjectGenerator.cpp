...
void say_hello()
{
	...
	void ScriptProjectGenerator::Generate(const std::string &templatePath, const std::string &projectName, const std::string &projectDirectory)
	{
		...
		{
			std::string fromDirectory = "Nexus";
			std::string toDirectory	  = scriptDirectory + "\\Nexus";

			if (std::filesystem::exists(fromDirectory))
			{
				if (!std::filesystem::exists(toDirectory))
				{
					FileSystem::CreateFileDirectory(toDirectory);
				}
				FileSystem::CopyDirectory("Nexus", toDirectory, true);
			}
		}
	}
}	 // namespace Nexus::Utils