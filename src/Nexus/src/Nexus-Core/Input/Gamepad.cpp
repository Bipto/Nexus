...
void Gamepad::Update() {
  ...
  m_LeftStick.X = SDL_GetGamepadAxis(m_GameController,
                                     SDL_GamepadAxis::SDL_GAMEPAD_AXIS_LEFTX);
  m_LeftStick.Y = SDL_GetGamepadAxis(m_GameController,
                                     SDL_GamepadAxis::SDL_GAMEPAD_AXIS_LEFTY);
  m_RightStick.X = SDL_GetGamepadAxis(m_GameController,
                                      SDL_GamepadAxis::SDL_GAMEPAD_AXIS_RIGHTX);
  m_RightStick.Y = SDL_GetGamepadAxis(m_GameController,
                                      SDL_GamepadAxis::SDL_GAMEPAD_AXIS_RIGHTY);
  m_LeftTrigger = SDL_GetGamepadAxis(
      m_GameController, SDL_GamepadAxis::SDL_GAMEPAD_AXIS_LEFT_TRIGGER);
  m_RightTrigger = SDL_GetGamepadAxis(
      m_GameController, SDL_GamepadAxis::SDL_GAMEPAD_AXIS_RIGHT_TRIGGER);
...
const float Gamepad::GetLeftTrigger() const { return m_LeftTrigger / 32767.0f; }
...
const int Gamepad::GetDeadzone() const { return m_Deadzone; }

void Gamepad::SetDeadzone(const int deadzone) { m_Deadzone = deadzone; }
...
void Gamepad::Rumble(uint16_t lowFrequency, uint16_t highFrequency,
                     uint32_t milliseconds) {
  SDL_RumbleGamepad(m_GameController, lowFrequency, highFrequency,
                    milliseconds);
}

void Gamepad::RumbleTriggers(uint16_t left, uint16_t right,
                             uint32_t milliseconds) {
  SDL_RumbleGamepadTriggers(m_GameController, left, right, milliseconds);
}

const uint32_t Gamepad::GetControllerIndex() { return m_Index; }
