...
const std::string s_BatchFontFragmentShaderSource =
    "#version 450 core\n"
...
Nexus::Graphics::ResourceSetSpecification GetResourceSetSpecification() {
  ...
  resourceSpec.SampledImages = {
      ...
      {"texture15", 1, 15,
       Nexus::Graphics::ResourceType::CombinedImageSampler}};
...
bool FindTextureInBatch(BatchInfo &info, Ref<Texture2D> texture,
                        uint32_t &index) {
...
float GetOrCreateTexIndex(BatchInfo &info,
                          Nexus::Ref<Nexus::Graphics::Texture2D> texture) {
  uint32_t index = 0;
...
void FlushTextures(BatchInfo &info,
                   Nexus::Ref<Nexus::Graphics::Texture2D> blankTexture) {
  info.Textures.clear();
  info.Textures.push_back(blankTexture);
}

void ResetBatcher(BatchInfo &info,
                  Nexus::Ref<Nexus::Graphics::Texture2D> blankTexture) {
  info.Vertices.clear();
  info.Indices.clear();
  info.VertexCount = 0;
...
void CreateBatcher(BatchInfo &info, Nexus::Graphics::GraphicsDevice *device,
  ...
  description.RasterizerStateDesc.TriangleCullMode =
      Nexus::Graphics::CullMode::None;
  description.Layouts = {
      Nexus::Graphics::VertexPositionTexCoordColorTexIndex::GetLayout()};
  ...
  description.BlendStateDesc.DestinationColourBlend =
  ...
  description.BlendStateDesc.DestinationAlphaBlend =
      BlendFactor::OneMinusSourceAlpha;
  description.BlendStateDesc.AlphaBlendFunction = BlendEquation::Add;

  description.ColourTargetSampleCount =
      device->GetPrimaryWindow()->GetSwapchain()->GetSpecification().Samples;
  ...
  vertexBufferDesc.Size =
      info.Vertices.size() * sizeof(VertexPositionTexCoordColorTexIndex);
...
BatchRenderer::BatchRenderer(Nexus::Graphics::GraphicsDevice *device,
                             Nexus::Graphics::RenderTarget target)
  ...
  Nexus::Ref<Nexus::Graphics::ShaderModule> vertexModule =
      device->CreateShaderModuleFromSpirvSource(
          s_BatchVertexShaderSource, "Batch Renderer - Vertex Shader",
  ...
  CreateBatcher(m_SDFBatchInfo, device, target, vertexModule,
                sdfFragmentModule);
  CreateBatcher(m_TextureBatchInfo, device, target, vertexModule,
                textureFragmentModule);
  CreateBatcher(m_FontBatchInfo, device, target, vertexModule,
                fontFragmentModule);
...
void BatchRenderer::Begin(Viewport viewport, Scissor scissor) {
  ...
  glm::mat4 projection =
      glm::ortho<float>(m_Viewport.X, m_Viewport.Width, m_Viewport.Height,
                        m_Viewport.Y, -1.0f, 1.0f);
  m_UniformBuffer->SetData(&projection, sizeof(projection));
}

void BatchRenderer::DrawQuadFill(const glm::vec2 &min, const glm::vec2 &max,
...
void BatchRenderer::DrawQuadFill(const Rectangle<float> &rectangle,
                                 const glm::vec4 &color) {
...
void BatchRenderer::DrawQuad(const glm::vec2 &min, const glm::vec2 &max,
...
void BatchRenderer::DrawQuad(const Rectangle<float> &rectangle,
                             const glm::vec4 &color, float thickness) {
  glm::vec2 min = {(float)rectangle.GetLeft(), (float)rectangle.GetTop()};
  glm::vec2 max = {(float)rectangle.GetRight(), (float)rectangle.GetBottom()};
...
void BatchRenderer::DrawCharacter(char character, const glm::vec2 &position,
...
void BatchRenderer::DrawString(const std::string &text,
                               const glm::vec2 &position, uint32_t size,
...
void BatchRenderer::DrawCircle(const glm::vec2 &position, float radius,
                               const glm::vec4 &color, uint32_t numberOfPoints,
                               float thickness) {
...
void BatchRenderer::DrawCircle(const Circle<float> &circle,
                               const glm::vec4 &color, uint32_t numberOfPoints,
                               float thickness) {
  const auto &circlePos = circle.GetPosition();
  DrawCircle({circlePos.X, circlePos.Y}, circle.GetRadius(), color,
             numberOfPoints, thickness);
}

void BatchRenderer::DrawCircleFill(const glm::vec2 &position, float radius,
                                   const glm::vec4 &color,
...
void BatchRenderer::DrawCircleRegionFill(const glm::vec2 &position,
                                         ...
                                         uint32_t numberOfPoints,
                                         float startAngle, float fillAngle) {
  DrawCircleRegionFill(position, radius, color, numberOfPoints, startAngle,
                       fillAngle, m_BlankTexture);
}
...
void BatchRenderer::DrawCircleRegionFill(const glm::vec2 &position,
  ...
  float currentAngle =
      ...
      glm::radians(180.0f); // start rendering the circle from the centre top
  ...
  glm::vec2 centreUV = {Nexus::Utils::ReMapRange(topLeft.x, bottomRight.x,
                                                 uvTL.x, uvBR.x, centre.x),
                        Nexus::Utils::ReMapRange(topLeft.y, bottomRight.y,
                                                 uvTL.y, uvBR.y, centre.y)};

  for (int i = 0; i < numberOfPoints; i++) {
    glm::vec3 posA = {glm::sin(currentAngle) * radius + (position.x),
                      glm::cos(currentAngle) * radius + (position.y), 0.0f};

    glm::vec2 uvA = {Nexus::Utils::ReMapRange(topLeft.x, bottomRight.x, uvTL.x,
                                              uvBR.x, posA.x),
                     Nexus::Utils::ReMapRange(topLeft.y, bottomRight.y, uvTL.y,
                                              uvBR.y, posA.y)};
    ...
    glm::vec2 uvB = {Nexus::Utils::ReMapRange(topLeft.x, bottomRight.x, uvTL.x,
                                              uvBR.x, posB.x),
                     Nexus::Utils::ReMapRange(topLeft.y, bottomRight.y, uvTL.y,
                                              uvBR.y, posB.y)};
...
void BatchRenderer::DrawCircleFill(const Circle<float> &circle,
  ...
  DrawCircleRegionFill({circle.GetPosition().X, circle.GetPosition().Y},
                       circle.GetRadius(), color, numberOfPoints, 0.0f, 360.0f,
                       texture, tilingFactor);
}

void BatchRenderer::DrawCross(const Rectangle<float> &rectangle,
                              float thickness, const glm::vec4 &color) {
  glm::vec2 topLeft = {rectangle.GetLeft() + thickness,
                       rectangle.GetTop() + thickness};
  glm::vec2 bottomRight = {rectangle.GetRight() - thickness,
                           rectangle.GetBottom() - thickness};
  glm::vec2 bottomLeft = {rectangle.GetLeft() + thickness,
                          rectangle.GetBottom() - thickness};
  glm::vec2 topRight = {rectangle.GetRight() - thickness,
                        rectangle.GetTop() + thickness};
...
void BatchRenderer::DrawTriangle(const glm::vec3 &pos0, const glm::vec2 &uv0,
                                 const glm::vec3 &pos1, const glm::vec2 &uv1,
                                 const glm::vec3 &pos2, const glm::vec2 &uv2,
                                 const glm::vec4 &color) {
...
void BatchRenderer::DrawTriangle(const Graphics::Triangle2D &tri,
                                 const glm::vec4 &color) {
  DrawTriangle({tri.A.x, tri.A.y, 0.0f}, {0.0f, 0.0f}, {tri.B.x, tri.B.y, 0.0f},
               {0.0f, 0.0f}, {tri.C.x, tri.C.y, 0.0f}, {0.0f, 0.0f}, color);
}

void BatchRenderer::DrawPolygonFill(const Polygon &polygon,
                                    const glm::vec4 &color) {
...
void BatchRenderer::DrawPolygonFill(const Polygon &polygon,
  ...
  for (const auto &triangle2D : tris) {
    ...
    const glm::vec2 uvA = {
        Nexus::Utils::ReMapRange(boundingRectangle.GetLeft(),
                                 boundingRectangle.GetRight(), uvTL.x, uvBR.x,
                                 triToDraw.A.x),
        Nexus::Utils::ReMapRange(boundingRectangle.GetTop(),
                                 boundingRectangle.GetBottom(), uvTL.y, uvBR.y,
                                 triToDraw.A.y)};
    ...
    DrawTriangle(triToDraw.A, uvA, triToDraw.B, uvB, triToDraw.C, uvC, color,
                 texture);
  }
}
...
void BatchRenderer::EnsureSpace(BatchInfo &info, uint32_t shapeVertexCount,
                                uint32_t shapeIndexCount) {
  ...
  if (shapeVertexCount > maxVertexCount || shapeIndexCount > maxIndexCount) {
    throw std::runtime_error(
        "Max vertex or index count reached for one draw call");
  }

  if (info.VertexCount + shapeVertexCount > maxVertexCount ||
      info.IndexCount + shapeIndexCount > maxIndexCount ||
...
void BatchRenderer::PerformDraw(BatchInfo &info) {
  ...
  info.VertexBuffer->SetData(info.Vertices.data(),
                             info.Vertices.size() *
                                 sizeof(VertexPositionTexCoordColorTexIndex));
  info.IndexBuffer->SetData(info.Indices.data(),
  ...
  for (uint32_t i = 0; i < MAX_TEXTURE_COUNT; i++) {
    ...
    if (i < info.Textures.size()) {
      info.ResourceSet->WriteCombinedImageSampler(info.Textures.at(i),
                                                  m_Sampler, textureName);
    } else {
      info.ResourceSet->WriteCombinedImageSampler(m_BlankTexture, m_Sampler,
                                                  textureName);
    }
  }
  ...
  m_CommandList->SetRenderTarget(Nexus::Graphics::RenderTarget(
      m_Device->GetPrimaryWindow()->GetSwapchain()));
