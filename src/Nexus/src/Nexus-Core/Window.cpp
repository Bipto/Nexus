...
Window::Window(const WindowSpecification &windowProps,
  ...
  m_Window = SDL_CreateWindow(windowProps.Title.c_str(), windowProps.Width,
                              windowProps.Height, flags);
...
#else
  SDL_SetWindowSize(m_Window, size.X * GetDisplayScale(),
                    size.Y * GetDisplayScale());
#endif
}

void Window::Close() { m_Closing = true; }

bool Window::IsClosing() { return m_Closing; }

SDL_Window *Window::GetSDLWindowHandle() { return m_Window; }
...
const InputState *Window::GetInput() { return &m_Input; }
...
void Window::SetFullscreen() { SDL_SetWindowFullscreen(m_Window, SDL_TRUE); }

void Window::UnsetFullscreen() { SDL_SetWindowFullscreen(m_Window, SDL_FALSE); }
...
Graphics::Swapchain *Window::GetSwapchain() { return m_Swapchain; }

uint32_t Window::GetID() { return m_WindowID; }

float Window::GetDisplayScale() { return SDL_GetWindowDisplayScale(m_Window); }
...
uint32_t
Window::GetFlags(Graphics::GraphicsAPI api,
