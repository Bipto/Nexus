...
Texture2D_Vk::Texture2D_Vk(GraphicsDeviceVk *graphicsDevice,
                           const Texture2DSpecification &spec)
    : Texture2D(spec, graphicsDevice), m_GraphicsDevice(graphicsDevice) {
  ...
  m_StagingBuffer = graphicsDevice->CreateBuffer(
      imageSize,
  ...
  if (vmaCreateImage(graphicsDevice->GetAllocator(), &imageInfo,
                     &imageAllocInfo, &m_Image, &m_Allocation,
                     nullptr) != VK_SUCCESS) {
  ...
  if (vkCreateImageView(graphicsDevice->GetVkDevice(), &createInfo, nullptr,
...
void Texture2D_Vk::SetData(const void *data, uint32_t level, uint32_t x,
  ...
  void *buffer;
  vmaMapMemory(m_GraphicsDevice->GetAllocator(), m_StagingBuffer.Allocation,
               &buffer);
  ...
  // upload texture to GPU
  {
    ...
    m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
      m_GraphicsDevice->TransitionVulkanImageLayout(
          cmd, m_Image, level, 0, before, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
      ...
      vkCmdCopyBufferToImage(cmd, m_StagingBuffer.Buffer, m_Image,
...
std::vector<std::byte> Texture2D_Vk::GetData(uint32_t level, uint32_t x,
  ...
  // retrieve pixels from texture
  {
    ...
    m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
      m_GraphicsDevice->TransitionVulkanImageLayout(
          cmd, m_Image, level, 0, before, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
          VK_IMAGE_ASPECT_COLOR_BIT);
      ...
      vkCmdCopyImageToBuffer(cmd, m_Image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                             m_StagingBuffer.Buffer, 1, &copyRegion);
...
VkImage Texture2D_Vk::GetImage() { return m_Image; }

VkImageView Texture2D_Vk::GetImageView() { return m_ImageView; }
...
Cubemap_Vk::Cubemap_Vk(GraphicsDeviceVk *graphicsDevice,
                       const CubemapSpecification &spec)
    : Cubemap(spec, graphicsDevice), m_GraphicsDevice(graphicsDevice) {
  uint32_t sizeInBits = GetPixelFormatSizeInBits(m_Specification.Format);
  VkDeviceSize imageSize =
      m_Specification.Width * m_Specification.Height * sizeInBits;
  ...
  VkExtent3D imageExtent{
      .width = spec.Width, .height = spec.Height, .depth = 1};
  ...
  for (uint32_t arrayLayer = 0; arrayLayer < 6; arrayLayer++) {
    ...
    for (uint32_t mipLevel = 0; mipLevel < m_Specification.MipLevels;
         mipLevel++) {
      layerLayouts.push_back(VK_IMAGE_LAYOUT_UNDEFINED);
    }
...
void Cubemap_Vk::SetData(const void *data, CubemapFace face, uint32_t level,
  ...
  // upload from staging buffer to GPU
  {
    ...
    m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
      m_GraphicsDevice->TransitionVulkanImageLayout(
          cmd, m_Image, level, faceIndex, before,
...
std::vector<std::byte> Cubemap_Vk::GetData(CubemapFace face, uint32_t level,
  ...
  // retrieve pixels from texture
  {
    ...
    vmaMapMemory(m_GraphicsDevice->GetAllocator(), m_StagingBuffer.Allocation,
                 &buffer);
    memcpy(pixels.data(), buffer, pixels.size());
    vmaUnmapMemory(m_GraphicsDevice->GetAllocator(),
...
VkImage Cubemap_Vk::GetImage() { return m_Image; }

VkImageView Cubemap_Vk::GetImageView() { return m_ImageView; }

VkImageLayout Cubemap_Vk::GetImageLayout(uint32_t arrayLayer,
                                         uint32_t mipLevel) {
  return m_Layouts.at(arrayLayer).at(mipLevel);
}

void Cubemap_Vk::SetImageLayout(uint32_t arrayLayer, uint32_t mipLevel,
