...
void Texture2D_Vk::SetData(const void *data, uint32_t level, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    // upload texture to GPU
    {
        ...
        m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
            m_GraphicsDevice->TransitionVulkanImageLayout(cmd, m_Image, level, 0, before, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
...
std::vector<std::byte> Texture2D_Vk::GetData(uint32_t level, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    // retrieve pixels from texture
    {
        ...
        m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
            m_GraphicsDevice->TransitionVulkanImageLayout(cmd, m_Image, level, 0, before, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
...
Cubemap_Vk::Cubemap_Vk(GraphicsDeviceVk *graphicsDevice, const CubemapSpecification &spec) : Cubemap(spec, graphicsDevice), m_GraphicsDevice(graphicsDevice)
{
...
void Cubemap_Vk::SetData(const void *data, CubemapFace face, uint32_t level, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    // upload from staging buffer to GPU
    {
        ...
        m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
            m_GraphicsDevice->TransitionVulkanImageLayout(cmd, m_Image, level, faceIndex, before, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
...
std::vector<std::byte> Cubemap_Vk::GetData(CubemapFace face, uint32_t level, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    // retrieve pixels from texture
    {
        ...
        m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
            m_GraphicsDevice->TransitionVulkanImageLayout(cmd, m_Image, level, faceIndex, before, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
