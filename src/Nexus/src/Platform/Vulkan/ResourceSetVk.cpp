...
ResourceSetVk::ResourceSetVk(const ResourceSetSpecification &spec,
  ...
  for (const auto &textureBinding : spec.SampledImages) {
    ...
    if (descriptorCounts.find(descriptorBinding.descriptorType) ==
  ...
  for (const auto &set : sets) {
    ...
    if (vkCreateDescriptorSetLayout(device->GetVkDevice(), &setInfo, nullptr,
                                    &layout) != VK_SUCCESS) {
  ...
  for (int frameIndex = 0; frameIndex < m_DescriptorSets.size(); frameIndex++) {
    ...
    for (int setIndex = 0; setIndex < m_DescriptorSetLayouts.size();
         setIndex++) {
      ...
      if (vkAllocateDescriptorSets(m_Device->GetVkDevice(), &allocInfo,
                                   &descriptorSets[setIndex]) != VK_SUCCESS) {
...
ResourceSetVk::~ResourceSetVk() {
  for (const auto &layout : m_DescriptorSetLayouts) {
    vkDestroyDescriptorSetLayout(m_Device->GetVkDevice(), layout.second,
                                 nullptr);
  }
...
void ResourceSetVk::WriteUniformBuffer(Ref<UniformBuffer> uniformBuffer,
                                       const std::string &name) {
  Ref<UniformBufferVk> uniformBufferVk =
      std::dynamic_pointer_cast<UniformBufferVk>(uniformBuffer);
  const auto &descriptorSets =
      m_DescriptorSets[m_Device->GetCurrentFrameIndex()];
  ...
  vkUpdateDescriptorSets(m_Device->GetVkDevice(), 1, &uniformBufferToWrite, 0,
                         nullptr);

  m_BoundUniformBuffers[name] = uniformBuffer;
}

void ResourceSetVk::WriteCombinedImageSampler(Ref<Texture2D> texture,
                                              Ref<Sampler> sampler,
                                              const std::string &name) {
  Ref<Texture2D_Vk> textureVk =
      std::dynamic_pointer_cast<Texture2D_Vk>(texture);
  ...
  m_Device->ImmediateSubmit([&](VkCommandBuffer cmd) {
    for (uint32_t i = 0; i < textureVk->GetLevels(); i++) {
      m_Device->TransitionVulkanImageLayout(
          cmd, textureVk->GetImage(), i, 0, textureVk->GetImageLayout(i),
  ...
  vkUpdateDescriptorSets(m_Device->GetVkDevice(), 1, &textureToWrite, 0,
                         nullptr);
...
void ResourceSetVk::WriteCombinedImageSampler(Ref<Cubemap> cubemap,
                                              Ref<Sampler> sampler,
                                              const std::string &name) {
  ...
  m_Device->ImmediateSubmit([&](VkCommandBuffer cmd) {
    for (uint32_t arrayLayer = 0; arrayLayer < 6; arrayLayer++) {
      for (uint32_t mipLevel = 0; mipLevel < cubemapVk->GetLevels();
           mipLevel++) {
        m_Device->TransitionVulkanImageLayout(
            cmd, cubemapVk->GetImage(), mipLevel, arrayLayer,
            cubemapVk->GetImageLayout(arrayLayer, mipLevel),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
            VK_IMAGE_ASPECT_COLOR_BIT);
        cubemapVk->SetImageLayout(arrayLayer, mipLevel,
                                  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
      }
    }
...
ResourceSetVk::GetDescriptorSetLayouts() const {
  return m_DescriptorSetLayouts;
}

const std::vector<std::map<uint32_t, VkDescriptorSet>> &
ResourceSetVk::GetDescriptorSets() const {
