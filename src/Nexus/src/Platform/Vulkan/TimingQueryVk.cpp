...
TimingQueryVk::TimingQueryVk(GraphicsDeviceVk *device) : m_Device(device) {
  ...
  vkCreateQueryPool(m_Device->GetVkDevice(), &queryPoolInfo, nullptr,
                    &m_QueryPool);
}
...
void TimingQueryVk::Resolve() {
  ...
  vkGetQueryPoolResults(m_Device->GetVkDevice(), m_QueryPool, 0, 2,
...
float TimingQueryVk::GetElapsedMilliseconds() { return m_ElapsedTime; }

VkQueryPool TimingQueryVk::GetQueryPool() { return m_QueryPool; }
} // namespace Nexus::Graphics
