...
void SwapchainVk::CreateDepthStencil()
{
    ...
    CreateImage(m_SwapchainSize.width, m_SwapchainSize.height, VK_FORMAT_D24_UNORM_S8_UINT, VK_IMAGE_TILING_OPTIMAL,
...
void SwapchainVk::CreateResolveAttachment()
{
    ...
    CreateImage(m_SwapchainSize.width, m_SwapchainSize.height, m_SurfaceFormat.format, VK_IMAGE_TILING_OPTIMAL,
...
{
    VkResult result =
        vkAcquireNextImageKHR(m_GraphicsDevice->GetVkDevice(), m_Swapchain, 0, m_PresentSemaphores[m_GraphicsDevice->GetCurrentFrameIndex()], VK_NULL_HANDLE, &m_CurrentFrameIndex);
...
void SwapchainVk::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage &image,
