...
SwapchainVk::SwapchainVk(Window *window, GraphicsDevice *graphicsDevice,
...
void SwapchainVk::SwapBuffers() {
  ...
  m_GraphicsDevice->ImmediateSubmit([&](VkCommandBuffer cmd) {
    m_GraphicsDevice->TransitionVulkanImageLayout(
        cmd, GetColourImage(), 0, 0, GetColorImageLayout(),
  ...
  VkResult result =
      vkQueuePresentKHR(m_GraphicsDevice->m_PresentQueue, &presentInfo);
...
VSyncState SwapchainVk::GetVsyncState() { return m_VsyncState; }
...
VkSurfaceFormatKHR SwapchainVk::GetSurfaceFormat() { return m_SurfaceFormat; }

VkFormat SwapchainVk::GetDepthFormat() { return m_DepthFormat; }
...
uint32_t SwapchainVk::GetImageCount() { return m_SwapchainImageCount; }

VkExtent2D SwapchainVk::GetSwapchainSize() const { return m_SwapchainSize; }

VkImage SwapchainVk::GetColourImage() {
  return m_SwapchainImages[m_CurrentFrameIndex];
}

VkImage SwapchainVk::GetDepthImage() { return m_DepthImage; }

VkImage SwapchainVk::GetResolveImage() { return m_ResolveImage; }

VkImageView SwapchainVk::GetColourImageView() {
  return m_SwapchainImageViews[m_CurrentFrameIndex];
}

VkImageView SwapchainVk::GetDepthImageView() { return m_DepthImageView; }

VkImageView SwapchainVk::GetResolveImageView() { return m_ResolveImageView; }

VkImageLayout SwapchainVk::GetColorImageLayout() {
  return m_ImageLayouts[m_CurrentFrameIndex];
}

VkImageLayout SwapchainVk::GetDepthImageLayout() { return m_DepthLayout; }
...
bool SwapchainVk::IsSwapchainValid() const { return m_SwapchainValid; }
...
void SwapchainVk::CreateSurface() {
  if (!SDL_Vulkan_CreateSurface(m_Window->GetSDLWindowHandle(),
                                m_GraphicsDevice->m_Instance, nullptr,
                                &m_Surface)) {
...
bool SwapchainVk::CreateSwapchain() {
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_GraphicsDevice->m_PhysicalDevice,
                                            m_Surface, &m_SurfaceCapabilities);

  std::vector<VkSurfaceFormatKHR> surfaceFormats;
  uint32_t surfaceFormatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(m_GraphicsDevice->m_PhysicalDevice,
                                       m_Surface, &surfaceFormatCount, nullptr);
  ...
  if (!found) {
    throw std::runtime_error(
        "Could not find surface format: VK_FORMAT_R8G8B8A8_UNORM");
  }
  ...
  // &height);
  width = std::clamp(width, (int)m_SurfaceCapabilities.minImageExtent.width,
                     (int)m_SurfaceCapabilities.maxImageExtent.width);
  height = std::clamp(height, (int)m_SurfaceCapabilities.minImageExtent.height,
                      (int)m_SurfaceCapabilities.maxImageExtent.height);
  ...
  uint32_t queueFamilyIndices[] = {m_GraphicsDevice->m_GraphicsQueueFamilyIndex,
                                   m_GraphicsDevice->m_PresentQueueFamilyIndex};
  if (m_SurfaceCapabilities.maxImageCount > 0 &&
      imageCount > m_SurfaceCapabilities.maxImageCount) {
    imageCount = m_SurfaceCapabilities.maxImageCount;
  }
  ...
  if (vkCreateSwapchainKHR(m_GraphicsDevice->m_Device, &createInfo, nullptr,
                           &m_Swapchain) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create swapchain");
  }

  vkGetSwapchainImagesKHR(m_GraphicsDevice->m_Device, m_Swapchain,
...
void SwapchainVk::CreateSwapchainImageViews() {
  ...
  for (uint32_t i = 0; i < m_SwapchainImages.size(); i++) {
    m_SwapchainImageViews[i] =
        CreateImageView(m_SwapchainImages[i], m_SurfaceFormat.format,
...
void SwapchainVk::CreateDepthStencil() {
  ...
  VkBool32 validDepthFormat = GetSupportedDepthFormat(
      m_GraphicsDevice->m_PhysicalDevice, &m_DepthFormat);
...
void SwapchainVk::CreateResolveAttachment() {
  ...
  CreateImage(m_SwapchainSize.width, m_SwapchainSize.height,
              m_SurfaceFormat.format, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT |
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_ResolveImage,
              m_ResolveMemory, samples);

  m_ResolveImageView = CreateImageView(m_ResolveImage, m_SurfaceFormat.format,
                                       VK_IMAGE_ASPECT_COLOR_BIT);
...
void SwapchainVk::CreateSemaphores() {
  for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; i++) {
    ...
    if (vkCreateSemaphore(m_GraphicsDevice->GetVkDevice(), &semaphoreCreateInfo,
...
void SwapchainVk::CleanupSwapchain() {
  ...
  for (size_t i = 0; i < m_SwapchainImageViews.size(); i++) {
    vkDestroyImageView(m_GraphicsDevice->m_Device, m_SwapchainImageViews[i],
                       nullptr);
  }
...
void SwapchainVk::CleanupSemaphores() {
  for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(m_GraphicsDevice->GetVkDevice(), m_PresentSemaphores[i],
                       nullptr);
  }
}
...
bool SwapchainVk::AcquireNextImage() {
  VkResult result = vkAcquireNextImageKHR(
      m_GraphicsDevice->GetVkDevice(), m_Swapchain, 0,
      m_PresentSemaphores[m_GraphicsDevice->GetCurrentFrameIndex()],
...
VkImageView SwapchainVk::CreateImageView(VkImage image, VkFormat format,
  ...
  if (vkCreateImageView(m_GraphicsDevice->m_Device, &viewInfo, nullptr,
...
VkBool32 SwapchainVk::GetSupportedDepthFormat(VkPhysicalDevice physicalDevice,
...
void SwapchainVk::CreateImage(uint32_t width, uint32_t height, VkFormat format,
  ...
  if (vkCreateImage(m_GraphicsDevice->m_Device, &imageInfo, nullptr, &image) !=
      VK_SUCCESS) {
    throw std::runtime_error("Failed to create image");
  }

  VkMemoryRequirements memRequirements;
  vkGetImageMemoryRequirements(m_GraphicsDevice->m_Device, image,
                               &memRequirements);
  ...
  if (vkAllocateMemory(m_GraphicsDevice->m_Device, &allocInfo, nullptr,
...
uint32_t SwapchainVk::FindMemoryType(uint32_t typeFilter,
                                     VkMemoryPropertyFlags properties) {
  VkPhysicalDeviceMemoryProperties memProperties;
  vkGetPhysicalDeviceMemoryProperties(m_GraphicsDevice->m_PhysicalDevice,
                                      &memProperties);
