...
void CommandExecutorVk::ExecuteCommand(SetVertexBufferCommand command,
  ...
  auto vulkanVB =
      std::dynamic_pointer_cast<VertexBufferVk>(command.VertexBufferRef.lock());

  VkBuffer vertexBuffers[] = {vulkanVB->GetBuffer()};
  VkDeviceSize offsets[] = {0};
  vkCmdBindVertexBuffers(m_CommandBuffer, command.Slot, 1, vertexBuffers,
                         offsets);
}

void CommandExecutorVk::ExecuteCommand(WeakRef<IndexBuffer> command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
...
void CommandExecutorVk::ExecuteCommand(WeakRef<Pipeline> command,
                                       GraphicsDevice *device) {
  if (command.expired()) {
...
void CommandExecutorVk::ExecuteCommand(DrawElementCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
...
void CommandExecutorVk::ExecuteCommand(DrawIndexedCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
  ...
  vkCmdDrawIndexed(m_CommandBuffer, command.Count, 1, command.IndexStart,
                   command.VertexStart, 0);
}

void CommandExecutorVk::ExecuteCommand(DrawInstancedCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
  ...
  vkCmdDraw(m_CommandBuffer, command.VertexCount, command.InstanceCount,
            command.VertexStart, command.InstanceStart);
}

void CommandExecutorVk::ExecuteCommand(DrawInstancedIndexedCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
  ...
  vkCmdDrawIndexed(m_CommandBuffer, command.IndexCount, command.InstanceCount,
                   command.IndexStart, command.VertexStart,
                   command.InstanceStart);
}

void CommandExecutorVk::ExecuteCommand(Ref<ResourceSet> command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
  ...
  auto pipelineVk =
      std::dynamic_pointer_cast<PipelineVk>(m_CurrentlyBoundPipeline);
  auto resourceSetVk = std::dynamic_pointer_cast<ResourceSetVk>(command);

  const auto &descriptorSets =
      resourceSetVk->GetDescriptorSets()[m_Device->GetCurrentFrameIndex()];
  for (const auto &set : descriptorSets) {
    vkCmdBindDescriptorSets(m_CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            pipelineVk->GetPipelineLayout(), set.first, 1,
                            &set.second, 0, nullptr);
  }
}

void CommandExecutorVk::ExecuteCommand(ClearColorTargetCommand command,
                                       GraphicsDevice *device) {
  ...
  clearAttachment.clearValue.color = {command.Color.Red, command.Color.Green,
...
void CommandExecutorVk::ExecuteCommand(ClearDepthStencilTargetCommand command,
                                       GraphicsDevice *device) {
...
void CommandExecutorVk::ExecuteCommand(RenderTarget command,
                                       GraphicsDevice *device) {
  if (m_Rendering) {
  ...
  m_RenderSize = {m_CurrentRenderTarget.GetSize().X,
                  m_CurrentRenderTarget.GetSize().Y};
  ...
  if (m_CurrentRenderTarget.GetType() == RenderTargetType::Swapchain) {
    ...
    auto vulkanFramebuffer =
        std::dynamic_pointer_cast<FramebufferVk>(framebuffer);

    StartRenderingToFramebuffer(vulkanFramebuffer);
  }
}

void CommandExecutorVk::ExecuteCommand(const Viewport &command,
                                       GraphicsDevice *device) {
...
void CommandExecutorVk::ExecuteCommand(const Scissor &command,
                                       GraphicsDevice *device) {
...
void CommandExecutorVk::ExecuteCommand(ResolveSamplesToSwapchainCommand command,
                                       GraphicsDevice *device) {
  ...
  auto framebufferVk =
      std::dynamic_pointer_cast<FramebufferVk>(command.Source.lock());
  auto swapchainVk = (SwapchainVk *)command.Target;

  VkImage framebufferImage =
      framebufferVk->GetVulkanColorTexture(command.SourceIndex)->GetImage();
  ...
  resolve.extent = {framebufferVk->GetFramebufferSpecification().Width,
                    framebufferVk->GetFramebufferSpecification().Height, 1};
  resolve.srcOffset = {0, 0, 0};
  resolve.srcSubresource = src;

  auto framebufferLayout =
      framebufferVk->GetVulkanColorTexture(command.SourceIndex)
          ->GetImageLayout(0);
  auto swapchainLayout = swapchainVk->GetColorImageLayout();

  vkCmdResolveImage(m_CommandBuffer, framebufferImage, framebufferLayout,
                    swapchainImage, swapchainLayout, 1, &resolve);

  ExecuteCommand(m_CurrentRenderTarget, device);
}

void CommandExecutorVk::ExecuteCommand(StartTimingQueryCommand command,
                                       GraphicsDevice *device) {
  Ref<TimingQueryVk> queryVk =
      std::dynamic_pointer_cast<TimingQueryVk>(command.Query.lock());
...
void CommandExecutorVk::ExecuteCommand(StopTimingQueryCommand command,
                                       GraphicsDevice *device) {
  Ref<TimingQueryVk> queryVk =
      std::dynamic_pointer_cast<TimingQueryVk>(command.Query.lock());
...
void CommandExecutorVk::ExecuteCommand(SetStencilRefCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
...
void CommandExecutorVk::ExecuteCommand(SetDepthBoundsCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
...
void CommandExecutorVk::ExecuteCommand(SetBlendFactorCommand command,
                                       GraphicsDevice *device) {
  if (!ValidateForGraphicsCall(m_CurrentlyBoundPipeline,
                               m_CurrentRenderTarget) ||
      !ValidateIsRendering()) {
...
void CommandExecutorVk::StartRenderingToSwapchain(SwapchainVk *swapchain) {
  m_Device->TransitionVulkanImageLayout(
      m_CommandBuffer, swapchain->GetColourImage(), 0, 0,
      swapchain->GetColorImageLayout(),
      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);

  m_Device->TransitionVulkanImageLayout(
      m_CommandBuffer, swapchain->GetDepthImage(), 0, 0,
  ...
  if (swapchain->GetSpecification().Samples == SampleCount::SampleCount1) {
    ...
    m_Device->TransitionVulkanImageLayout(
        m_CommandBuffer, swapchain->GetResolveImage(), 0, 0,
...
void CommandExecutorVk::StartRenderingToFramebuffer(
  ...
  Ref<FramebufferVk> vulkanFramebuffer =
      std::dynamic_pointer_cast<FramebufferVk>(framebuffer);

  // transition colour layouts
  for (size_t textureIndex = 0;
       textureIndex < vulkanFramebuffer->GetColorTextureCount();
       textureIndex++) {
    Ref<Texture2D_Vk> texture =
        vulkanFramebuffer->GetVulkanColorTexture(textureIndex);

    for (uint32_t mipLevel = 0; mipLevel < texture->GetLevels(); mipLevel++) {
      m_Device->TransitionVulkanImageLayout(
          m_CommandBuffer, texture->GetImage(), mipLevel, 0,
          texture->GetImageLayout(mipLevel),
          VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_ASPECT_COLOR_BIT);
      texture->SetImageLayout(mipLevel,
  ...
  VkRect2D renderArea{};
  renderArea.offset = {0, 0};
  renderArea.extent = {vulkanFramebuffer->GetFramebufferSpecification().Width,
  ...
  for (uint32_t colourAttachmentIndex = 0;
       colourAttachmentIndex < vulkanFramebuffer->GetColorTextureCount();
       colourAttachmentIndex++) {
    Ref<Texture2D_Vk> texture =
        vulkanFramebuffer->GetVulkanColorTexture(colourAttachmentIndex);
...
void CommandExecutorVk::StopRendering() {
  if (m_Rendering) {
    ...
    if (Nexus::Ref<Nexus::Graphics::Framebuffer> *framebuffer =
            m_CurrentRenderTarget.GetDataIf<Ref<Framebuffer>>()) {
...
void CommandExecutorVk::TransitionFramebufferToShaderReadonly(
    Ref<Framebuffer> framebuffer) {
  Ref<FramebufferVk> vulkanFramebuffer =
      std::dynamic_pointer_cast<FramebufferVk>(framebuffer);

  for (uint32_t colourAttachment = 0;
       colourAttachment < vulkanFramebuffer->GetColorTextureCount();
       colourAttachment++) {
    Ref<Texture2D_Vk> texture =
        vulkanFramebuffer->GetVulkanColorTexture(colourAttachment);

    for (uint32_t level = 0; level < texture->GetLevels(); level++) {
      m_Device->TransitionVulkanImageLayout(
          m_CommandBuffer, texture->GetImage(), level, 0,
