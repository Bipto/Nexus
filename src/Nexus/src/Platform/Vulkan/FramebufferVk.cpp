...
void FramebufferVk::SetFramebufferSpecification(
...
Ref<Texture2D> FramebufferVk::GetDepthTexture() { return m_DepthAttachment; }
...
void FramebufferVk::CreateColorTargets() {
  ...
  for (int i = 0;
    ...
    auto texture = std::dynamic_pointer_cast<Texture2D_Vk>(
...
void FramebufferVk::CreateDepthTargets() {
  ...
  if (m_Specification.DepthAttachmentSpecification.DepthFormat !=
    ...
    m_DepthAttachment = std::dynamic_pointer_cast<Texture2D_Vk>(
        m_Device->CreateTexture2D(spec));
  }
}
} // namespace Nexus::Graphics
