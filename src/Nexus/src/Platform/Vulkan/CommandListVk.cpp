...
CommandListVk::CommandListVk(GraphicsDeviceVk *graphicsDevice,
  ...
  for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
    // create command pools
    {
      ...
      if (vkCreateCommandPool(m_Device->GetVkDevice(), &createInfo, nullptr,
                              &m_CommandPools[i]) != VK_SUCCESS) {
    ...
    // create command buffers
    {
      ...
      if (vkAllocateCommandBuffers(m_Device->GetVkDevice(), &allocateInfo,
  ...
  // create fences and semaphores
  {
    ...
    for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; i++) {
      if (vkCreateFence(m_Device->GetVkDevice(), &fenceCreateInfo, nullptr,
                        &m_RenderFences[i]) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create fence");
      }

      if (vkCreateSemaphore(m_Device->GetVkDevice(), &semaphoreCreateInfo,
                            nullptr, &m_RenderSemaphores[i]) != VK_SUCCESS) {
...
CommandListVk::~CommandListVk() {
  for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
    vkFreeCommandBuffers(m_Device->GetVkDevice(), m_CommandPools[i], 1,
                         &m_CommandBuffers[i]);
