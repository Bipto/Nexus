...
GraphicsDeviceVk::GraphicsDeviceVk(
    const GraphicsDeviceSpecification &createInfo, Window *window,
...
GraphicsDeviceVk::~GraphicsDeviceVk() {
  ...
  vkFreeCommandBuffers(m_Device, m_UploadContext.CommandPool, 1,
                       &m_UploadContext.CommandBuffer);
  vkDestroyCommandPool(m_Device, m_UploadContext.CommandPool, nullptr);

  for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
    vkFreeCommandBuffers(m_Device, m_Frames[i].CommandPool, 1,
...
void GraphicsDeviceVk::SubmitCommandList(Ref<CommandList> commandList) {
  ...
  Ref<CommandListVk> commandListVk =
      std::dynamic_pointer_cast<CommandListVk>(commandList);

  vkWaitForFences(m_Device, 1, &commandListVk->GetCurrentFence(), VK_TRUE, 0);
  vkResetFences(m_Device, 1, &commandListVk->GetCurrentFence());

  const std::vector<Nexus::Graphics::RenderCommandData> &commands =
      commandListVk->GetCommandData();
  ...
  if (vkQueueSubmit(m_GraphicsQueue, 1, &submitInfo,
                    commandListVk->GetCurrentFence()) != VK_SUCCESS) {
    throw std::runtime_error("Failed to submit queue");
  }

  vkWaitForFences(m_Device, 1, &commandListVk->GetCurrentFence(), VK_TRUE,
                  UINT32_MAX);
  vkResetFences(m_Device, 1, &commandListVk->GetCurrentFence());
}

const std::string GraphicsDeviceVk::GetAPIName() { return "Vulkan"; }

const char *GraphicsDeviceVk::GetDeviceName() { return nullptr; }

Ref<ShaderModule> GraphicsDeviceVk::CreateShaderModule(
    const ShaderModuleSpecification &moduleSpec,
    const ResourceSetSpecification &resources) {
  return CreateRef<ShaderModuleVk>(moduleSpec, resources, this);
}

Ref<Texture2D>
GraphicsDeviceVk::CreateTexture2D(const Texture2DSpecification &spec) {
...
Ref<Pipeline>
GraphicsDeviceVk::CreatePipeline(const PipelineDescription &description) {
  return CreateRef<PipelineVk>(description, this);
}

Ref<CommandList>
GraphicsDeviceVk::CreateCommandList(const CommandListSpecification &spec) {
  return CreateRef<CommandListVk>(this, spec);
}

Ref<VertexBuffer>
GraphicsDeviceVk::CreateVertexBuffer(const BufferDescription &description,
                                     const void *data) {
  return CreateRef<VertexBufferVk>(description, data, this);
}

Ref<IndexBuffer>
GraphicsDeviceVk::CreateIndexBuffer(const BufferDescription &description,
                                    const void *data,
...
Ref<UniformBuffer>
GraphicsDeviceVk::CreateUniformBuffer(const BufferDescription &description,
                                      const void *data) {
  return CreateRef<UniformBufferVk>(description, data, this);
}

Ref<ResourceSet>
GraphicsDeviceVk::CreateResourceSet(const ResourceSetSpecification &spec) {
  return CreateRef<ResourceSetVk>(spec, this);
}

Ref<Framebuffer>
GraphicsDeviceVk::CreateFramebuffer(const FramebufferSpecification &spec) {
...
VkDevice GraphicsDeviceVk::GetVkDevice() { return m_Device; }
...
VmaAllocator GraphicsDeviceVk::GetAllocator() { return m_Allocator; }

const VkPhysicalDeviceProperties &GraphicsDeviceVk::GetDeviceProperties() {
  return m_DeviceProperties;
}

void GraphicsDeviceVk::ImmediateSubmit(
  ...
  if (vkQueueSubmit(m_GraphicsQueue, 1, &submitInfo,
                    m_UploadContext.UploadFence) != VK_SUCCESS) {
    throw std::runtime_error("Failed to submit queue");
  }
...
void GraphicsDeviceVk::TransitionVulkanImageLayout(
    VkCommandBuffer cmdBuffer, VkImage image, uint32_t mipLevel,
    uint32_t arrayLayer, VkImageLayout oldLayout, VkImageLayout newLayout,
...
void GraphicsDeviceVk::CreateInstance() {
  if (enableValidationLayers && !CheckValidationLayerSupport()) {
    throw std::runtime_error(
        "Validation layers were requested, but were not available");
  }
...
void GraphicsDeviceVk::SelectQueueFamilies() {
  ...
  for (const auto &queueFamily : queueFamilyProperties) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(m_PhysicalDevice, i,
                                         swapchain->m_Surface, &presentSupport);
...
void GraphicsDeviceVk::CreateCommandStructures() {
  ...
  for (int i = 0; i < FRAMES_IN_FLIGHT; i++) {
    // create command pools
    {
      ...
      if (vkCreateCommandPool(m_Device, &createInfo, nullptr,
                              &m_Frames[i].CommandPool) != VK_SUCCESS) {
    ...
    // upload command pool
    {
      ...
      createInfo.queueFamilyIndex = m_GraphicsQueueFamilyIndex;
      if (vkCreateCommandPool(m_Device, &createInfo, nullptr,
                              &m_UploadContext.CommandPool) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create command pool");
      }
    }
...
void GraphicsDeviceVk::CreateSynchronisationStructures() {
  ...
  if (vkCreateFence(m_Device, &fenceCreateInfo, nullptr,
                    &m_UploadContext.UploadFence) != VK_SUCCESS) {
    throw std::runtime_error("Failed to create fence");
  }
}

VkImageView GraphicsDeviceVk::CreateImageView(VkImage image, VkFormat format,
...
void GraphicsDeviceVk::CreateImage(uint32_t width, uint32_t height,
...
VkResult GraphicsDeviceVk::CreateDebugUtilsMessengerEXT(
...
void GraphicsDeviceVk::DestroyDebugUtilsMessengerEXT(
...
std::vector<std::string> GraphicsDeviceVk::GetSupportedDeviceExtensions() {
  ...
  VkResult result = vkEnumerateDeviceExtensionProperties(
      m_PhysicalDevice, nullptr, &count, nullptr);

  std::vector<VkExtensionProperties> properties(count);
  result = vkEnumerateDeviceExtensionProperties(m_PhysicalDevice, nullptr,
...
Vk::AllocatedBuffer GraphicsDeviceVk::CreateBuffer(size_t allocSize,
  ...
  if (vmaCreateBuffer(m_Allocator, &bufferInfo, &vmaAllocInfo, &buffer.Buffer,
...
uint32_t GraphicsDeviceVk::FindMemoryType(uint32_t typeFilter,
                                          VkMemoryPropertyFlags properties) {
