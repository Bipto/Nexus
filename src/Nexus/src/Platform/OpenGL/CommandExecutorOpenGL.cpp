...
void CommandExecutorOpenGL::ExecuteCommand(DrawIndexedCommand command, GraphicsDevice *device)
{
    ...
    if (Ref<PipelineOpenGL> pipeline = std::dynamic_pointer_cast<PipelineOpenGL>(m_CurrentlyBoundPipeline.lock()))
    {
        ...
        glCall(glDrawElements(GL::GetTopology(pipeline->GetPipelineDescription().PrimitiveTopology), command.Count, m_IndexBufferFormat, (void *)offset));
    }
}
...
void CommandExecutorOpenGL::ExecuteCommand(DrawInstancedCommand command, GraphicsDevice *device)
{
    ...
    if (Ref<PipelineOpenGL> pipeline = std::dynamic_pointer_cast<PipelineOpenGL>(m_CurrentlyBoundPipeline.lock()))
    {
        ...
        glCall(glDrawArraysInstanced(GL::GetTopology(pipeline->GetPipelineDescription().PrimitiveTopology), command.VertexStart, command.VertexCount, command.InstanceCount));
    }
}
...
void CommandExecutorOpenGL::ExecuteCommand(DrawInstancedIndexedCommand command, GraphicsDevice *device)
{
    ...
    if (Ref<PipelineOpenGL> pipeline = std::dynamic_pointer_cast<PipelineOpenGL>(m_CurrentlyBoundPipeline.lock()))
    {
        ...
        glCall(glDrawElementsInstanced(GL::GetTopology(pipeline->GetPipelineDescription().PrimitiveTopology), command.IndexCount, m_IndexBufferFormat, (void *)offset,
...
void CommandExecutorOpenGL::ExecuteCommand(Ref<ResourceSet> command, GraphicsDevice *device)
{
    ...
    for (const auto [name, uniformBuffer] : uniformBufferBindings)
    {
        ...
        if (Ref<UniformBufferOpenGL> uniformBufferGL = std::dynamic_pointer_cast<UniformBufferOpenGL>(uniformBuffer.lock()))
        {
            ...
            glCall(glBindBufferRange(GL_UNIFORM_BUFFER, uniformBufferSlot, uniformBufferGL->GetHandle(), 0, uniformBufferGL->GetDescription().Size));
