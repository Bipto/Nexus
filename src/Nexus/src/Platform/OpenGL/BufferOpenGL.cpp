...
VertexBufferOpenGL::VertexBufferOpenGL(const BufferDescription &description,
                                       const void *data)
...
void VertexBufferOpenGL::Unbind() { glCall(glBindBuffer(GL_ARRAY_BUFFER, 0)); }

unsigned int VertexBufferOpenGL::GetHandle() { return m_Buffer; }
...
IndexBufferOpenGL::IndexBufferOpenGL(const BufferDescription &description,
  ...
  glCall(glBufferData(GL_ELEMENT_ARRAY_BUFFER, description.Size, data,
                      bufferUsage));
}
...
unsigned int IndexBufferOpenGL::GetHandle() { return m_Buffer; }

void IndexBufferOpenGL::SetData(const void *data, uint32_t size,
                                uint32_t offset) {
  glCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_Buffer));
  glCall(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, offset, size, data));
}

UniformBufferOpenGL::UniformBufferOpenGL(const BufferDescription &description,
                                         const void *data)
...
unsigned int UniformBufferOpenGL::GetHandle() { return m_Buffer; }

void UniformBufferOpenGL::Unbind() {
  glCall(glBindBuffer(GL_UNIFORM_BUFFER, 0));
}

void UniformBufferOpenGL::SetData(const void *data, uint32_t size,
                                  uint32_t offset) {
  glCall(glBindBuffer(GL_UNIFORM_BUFFER, m_Buffer));
