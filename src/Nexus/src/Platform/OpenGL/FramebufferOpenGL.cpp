...
FramebufferOpenGL::~FramebufferOpenGL() { DeleteTextures(); }
...
FramebufferOpenGL::GetFramebufferSpecification() {
  return m_Specification;
}

void FramebufferOpenGL::SetFramebufferSpecification(
...
void FramebufferOpenGL::CreateTextures() {
  ...
  for (int i = 0;
    ...
    auto texture = std::dynamic_pointer_cast<Texture2DOpenGL>(
  ...
  if (m_Specification.DepthAttachmentSpecification.DepthFormat !=
    ...
    auto glTexture =
        std::dynamic_pointer_cast<Texture2DOpenGL>(m_DepthAttachment);

    glCall(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                                  GL_TEXTURE_2D, glTexture->GetHandle(), 0));
  }
}
