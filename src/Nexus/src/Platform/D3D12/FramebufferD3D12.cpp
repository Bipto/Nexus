...
FramebufferD3D12::FramebufferD3D12(const FramebufferSpecification &spec,
...
FramebufferD3D12::~FramebufferD3D12() { Flush(); }

void FramebufferD3D12::SetFramebufferSpecification(
...
Ref<Texture2D> FramebufferD3D12::GetDepthTexture() { return m_DepthAttachment; }
...
FramebufferD3D12::GetColorAttachmentCPUHandles() {
...
void FramebufferD3D12::CreateAttachments() {
  for (int i = 0;
       i < m_Specification.ColorAttachmentSpecification.Attachments.size();
       i++) {
    const auto &colorAttachmentSpec =
        m_Specification.ColorAttachmentSpecification.Attachments.at(i);

    if (colorAttachmentSpec.TextureFormat == PixelFormat::None) {
      NX_ASSERT(
          0, "Pixel format cannot be PixelFormat::None for a color attachment");
    }
    ...
    auto texture = std::dynamic_pointer_cast<Texture2D_D3D12>(
        m_Device->CreateTexture2D(spec));
    m_ColorAttachments.push_back(texture);
  }

  if (m_Specification.DepthAttachmentSpecification.DepthFormat !=
      PixelFormat::None) {
    Nexus::Graphics::Texture2DSpecification spec;
    spec.Width = m_Specification.Width;
    spec.Height = m_Specification.Height;
    ...
    m_DepthAttachment = std::dynamic_pointer_cast<Texture2D_D3D12>(
        m_Device->CreateTexture2D(spec));
  }
}
...
void FramebufferD3D12::CreateRTVs() {
  ...
  colorDescriptorHeapDesc.NumDescriptors =
      m_Specification.ColorAttachmentSpecification.Attachments.size();
  colorDescriptorHeapDesc.NodeMask = 0;
  colorDescriptorHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
  d3d12Device->CreateDescriptorHeap(&colorDescriptorHeapDesc,
                                    IID_PPV_ARGS(&m_ColorDescriptorHeap));
  ...
  d3d12Device->CreateDescriptorHeap(&depthDescriptorHeapDesc,
                                    IID_PPV_ARGS(&m_DepthDescriptorHeap));
  ...
  for (uint32_t i = 0;
    ...
    rtvDesc.Format = D3D12::GetD3D12PixelFormat(
        m_Specification.ColorAttachmentSpecification.Attachments[i]
            .TextureFormat,
        false);
    ...
    d3d12Device->CreateRenderTargetView(resourceHandle.Get(), &rtvDesc,
                                        cpuHandle);

    auto incrementSize = d3d12Device->GetDescriptorHandleIncrementSize(
  ...
  if (m_Specification.DepthAttachmentSpecification.DepthFormat !=
    ...
    dsvDesc.Format = D3D12::GetD3D12PixelFormat(
        m_Specification.DepthAttachmentSpecification.DepthFormat, true);
    ...
    auto resourceHandle = texture->GetD3D12ResourceHandle();
    d3d12Device->CreateDepthStencilView(resourceHandle.Get(), &dsvDesc,
                                        m_DepthAttachmentCPUHandle);
  }
}
} // namespace Nexus::Graphics
