...
PipelineD3D12::PipelineD3D12(ID3D12Device10 *device,
...
void PipelineD3D12::CreateRootSignature() {
  ...
  for (int i = 0; i < m_Description.ResourceSetSpec.SampledImages.size(); i++) {
    ...
    uint32_t slot = ResourceSet::GetLinearDescriptorSlot(textureInfo.Set,
                                                         textureInfo.Binding);
  ...
  for (int i = 0; i < m_Description.ResourceSetSpec.UniformBuffers.size();
       i++) {
    const auto &uniformBufferInfo =
        m_Description.ResourceSetSpec.UniformBuffers.at(i);
    uint32_t slot = ResourceSet::GetLinearDescriptorSlot(
        uniformBufferInfo.Set, uniformBufferInfo.Binding);
    ...
    constantBufferRange.OffsetInDescriptorsFromTableStart =
        D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
  ...
  if (SUCCEEDED(D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1,
                                            &m_RootSignatureBlob,
                                            &errorBlob))) {
...
void PipelineD3D12::CreatePipeline() {
  ...
  for (uint32_t index = 0; index < m_Description.ColourTargetCount; index++) {
    DXGI_FORMAT colourFormat = D3D12::GetD3D12PixelFormat(
        m_Description.ColourFormats.at(index), false);
  ...
  if (m_Description.FragmentModule) {
    auto d3d12FragmentModule = std::dynamic_pointer_cast<ShaderModuleD3D12>(
        m_Description.FragmentModule);
    NX_ASSERT(d3d12FragmentModule->GetShaderStage() == ShaderStage::Fragment,
  ...
  if (m_Description.GeometryModule) {
    auto d3d12GeometryModule = std::dynamic_pointer_cast<ShaderModuleD3D12>(
        m_Description.GeometryModule);
    NX_ASSERT(d3d12GeometryModule->GetShaderStage() == ShaderStage::Geometry,
  ...
  if (m_Description.TesselationControlModule) {
    auto d3d12TesselationControlModule =
        std::dynamic_pointer_cast<ShaderModuleD3D12>(
            m_Description.TesselationControlModule);
    NX_ASSERT(d3d12TesselationControlModule->GetShaderStage() ==
  ...
  if (m_Description.TesselationEvaluationModule) {
    ...
    NX_ASSERT(d3d12TesselationEvaluationModule->GetShaderStage() ==
  ...
  if (m_Description.VertexModule) {
    auto d3d12VertexModule = std::dynamic_pointer_cast<ShaderModuleD3D12>(
        m_Description.VertexModule);
    NX_ASSERT(d3d12VertexModule->GetShaderStage() == ShaderStage::Vertex,
  ...
  pipelineDesc.PrimitiveTopologyType =
      D3D12::GetPipelineTopology(m_Description.PrimitiveTopology);
  ...
  DXGI_FORMAT depthFormat =
      D3D12::GetD3D12PixelFormat(m_Description.DepthFormat, true);
  ...
  HRESULT hr = m_Device->CreateGraphicsPipelineState(
      &pipelineDesc, IID_PPV_ARGS(&m_PipelineStateObject));
  if (FAILED(hr)) {
    _com_error error(hr);
    std::cout << "Failed to create pipeline state: " << error.ErrorMessage()
              << std::endl;
  }
}
...
D3D12_RASTERIZER_DESC PipelineD3D12::CreateRasterizerState() {
  ...
  desc.CullMode =
      D3D12::GetCullMode(m_Description.RasterizerStateDesc.TriangleCullMode);

  if (m_Description.RasterizerStateDesc.TriangleFrontFace ==
...
D3D12_BLEND_DESC PipelineD3D12::CreateBlendStateDesc() {
  ...
  desc.RenderTarget[0].SrcBlend =
      D3D12::GetBlendFunction(m_Description.BlendStateDesc.SourceColourBlend);
  desc.RenderTarget[0].DestBlend = D3D12::GetBlendFunction(
      m_Description.BlendStateDesc.DestinationColourBlend);
  desc.RenderTarget[0].BlendOp =
      D3D12::GetBlendEquation(m_Description.BlendStateDesc.ColorBlendFunction);
  desc.RenderTarget[0].SrcBlendAlpha =
      D3D12::GetBlendFunction(m_Description.BlendStateDesc.SourceAlphaBlend);
  desc.RenderTarget[0].DestBlendAlpha = D3D12::GetBlendFunction(
      m_Description.BlendStateDesc.DestinationAlphaBlend);
  desc.RenderTarget[0].BlendOpAlpha =
      D3D12::GetBlendEquation(m_Description.BlendStateDesc.AlphaBlendFunction);
...
D3D12_DEPTH_STENCIL_DESC PipelineD3D12::CreateDepthStencilDesc() {
  ...
  desc.DepthFunc = D3D12::GetComparisonFunction(
  ...
  auto stencilFailOp = D3D12::GetStencilOperation(
      m_Description.DepthStencilDesc.StencilFailOperation);
  auto stencilDepthFailOp = D3D12::GetStencilOperation(
      m_Description.DepthStencilDesc.StencilSuccessDepthFailOperation);
  auto stencilPassOp = D3D12::GetStencilOperation(
      m_Description.DepthStencilDesc.StencilSuccessDepthSuccessOperation);
  auto stencilFunc = D3D12::GetComparisonFunction(
      m_Description.DepthStencilDesc.StencilComparisonFunction);
