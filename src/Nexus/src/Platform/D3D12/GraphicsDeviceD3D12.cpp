...
GraphicsDeviceD3D12::GraphicsDeviceD3D12(
  ...
  if (SUCCEEDED(D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_12_0,
    ...
    if (SUCCEEDED(m_Device->CreateCommandQueue(
            &commandQueueDesc, IID_PPV_ARGS(&m_CommandQueue)))) {
      // create the fence
      if (SUCCEEDED(m_Device->CreateFence(m_FenceValue, D3D12_FENCE_FLAG_NONE,
                                          IID_PPV_ARGS(&m_Fence)))) {
        m_FenceEvent = CreateEvent(nullptr, false, false, nullptr);
      }

      m_Device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                       IID_PPV_ARGS(&m_UploadCommandAllocator));
...
void GraphicsDeviceD3D12::SubmitCommandList(Ref<CommandList> commandList) {
  Ref<CommandListD3D12> d3d12CommandList =
      std::dynamic_pointer_cast<CommandListD3D12>(commandList);

  const std::vector<Nexus::Graphics::RenderCommandData> &commands =
      d3d12CommandList->GetCommandData();
...
const std::string GraphicsDeviceD3D12::GetAPIName() { return {"D3D12"}; }

const char *GraphicsDeviceD3D12::GetDeviceName() { return nullptr; }

Ref<ShaderModule> GraphicsDeviceD3D12::CreateShaderModule(
...
Ref<Texture2D>
GraphicsDeviceD3D12::CreateTexture2D(const Texture2DSpecification &spec) {
  return CreateRef<Texture2D_D3D12>(this, spec);
}

Ref<Cubemap>
GraphicsDeviceD3D12::CreateCubemap(const CubemapSpecification &spec) {
  return CreateRef<Cubemap_D3D12>(spec, this);
}

Ref<Pipeline>
GraphicsDeviceD3D12::CreatePipeline(const PipelineDescription &description) {
  return CreateRef<PipelineD3D12>(m_Device.Get(), description);
}

Ref<CommandList>
GraphicsDeviceD3D12::CreateCommandList(const CommandListSpecification &spec) {
  return CreateRef<CommandListD3D12>(this, spec);
}

Ref<VertexBuffer>
GraphicsDeviceD3D12::CreateVertexBuffer(const BufferDescription &description,
                                        const void *data) {
  return CreateRef<VertexBufferD3D12>(this, description, data);
}

Ref<IndexBuffer>
GraphicsDeviceD3D12::CreateIndexBuffer(const BufferDescription &description,
                                       const void *data,
...
Ref<UniformBuffer>
GraphicsDeviceD3D12::CreateUniformBuffer(const BufferDescription &description,
                                         const void *data) {
  return CreateRef<UniformBufferD3D12>(this, description, data);
}

Ref<ResourceSet>
GraphicsDeviceD3D12::CreateResourceSet(const ResourceSetSpecification &spec) {
  return CreateRef<ResourceSetD3D12>(spec, this);
}

Ref<Framebuffer>
GraphicsDeviceD3D12::CreateFramebuffer(const FramebufferSpecification &spec) {
  return CreateRef<FramebufferD3D12>(spec, this);
}

Ref<Sampler>
GraphicsDeviceD3D12::CreateSampler(const SamplerSpecification &spec) {
...
GraphicsDeviceD3D12::GetGraphicsCapabilities() const {
...
void GraphicsDeviceD3D12::ImmediateSubmit(
    std::function<void(ID3D12GraphicsCommandList7 *cmd)> &&function) {
...
void GraphicsDeviceD3D12::ResourceBarrier(ID3D12GraphicsCommandList7 *cmd,
                                          ...
                                          uint32_t level,
                                          D3D12_RESOURCE_STATES after) {
  ResourceBarrier(cmd, resource->GetD3D12ResourceHandle().Get(), level,
                  resource->GetResourceState(level), after);
  resource->SetResourceState(level, after);
}

void GraphicsDeviceD3D12::ResourceBarrierSwapchainColour(
    ID3D12GraphicsCommandList7 *cmd, SwapchainD3D12 *resource,
    D3D12_RESOURCE_STATES after) {
  ResourceBarrier(cmd, resource->RetrieveBufferHandle().Get(), 0,
                  resource->GetCurrentTextureState(), after);
  resource->SetTextureState(after);
}

void GraphicsDeviceD3D12::ResourceBarrierSwapchainDepth(
    ID3D12GraphicsCommandList7 *cmd, SwapchainD3D12 *resource,
    D3D12_RESOURCE_STATES after) {
  ResourceBarrier(cmd, resource->RetrieveDepthBufferHandle(), 0,
                  resource->GetCurrentDepthState(),
