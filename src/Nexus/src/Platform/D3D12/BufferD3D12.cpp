...
{
VertexBufferD3D12::VertexBufferD3D12(GraphicsDeviceD3D12 *device, const BufferDescription &description, const void *data) : VertexBuffer(description, data), m_Device(device)
{
...
void VertexBufferD3D12::SetData(const void *data, uint32_t size, uint32_t offset)
{
    ...
    m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) { cmd->CopyBufferRegion(m_VertexBuffer.Get(), 0, m_UploadBuffer.Get(), 0, m_Description.Size); });
}
...
void IndexBufferD3D12::SetData(const void *data, uint32_t size, uint32_t offset)
{
    ...
    m_UploadBuffer->Unmap(0, &m_UploadRange);
    m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) { cmd->CopyBufferRegion(m_IndexBuffer.Get(), 0, m_UploadBuffer.Get(), 0, m_Description.Size); });
}

UniformBufferD3D12::UniformBufferD3D12(GraphicsDeviceD3D12 *device, const BufferDescription &description, const void *data) : UniformBuffer(description, data), m_Device(device)
{
    auto d3d12Device = device->GetDevice();
...
void UniformBufferD3D12::SetData(const void *data, uint32_t size, uint32_t offset)
{
    ...
    m_UploadBuffer->Unmap(0, &m_UploadRange);
    m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) { cmd->CopyBufferRegion(m_ConstantBuffer.Get(), 0, m_UploadBuffer.Get(), 0, m_Description.Size); });
}
} // namespace Nexus::Graphics
#endif