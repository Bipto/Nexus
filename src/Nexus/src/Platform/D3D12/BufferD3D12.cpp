...
VertexBufferD3D12::VertexBufferD3D12(GraphicsDeviceD3D12 *device,
  ...
  d3d12Device->CreateCommittedResource(&bufferProperties, D3D12_HEAP_FLAG_NONE,
...
void VertexBufferD3D12::SetData(const void *data, uint32_t size,
                                uint32_t offset) {
  void *buffer;
  ...
  m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
    cmd->CopyBufferRegion(m_VertexBuffer.Get(), 0, m_UploadBuffer.Get(), 0,
                          m_Description.Size);
  });
}

ID3D12Resource2 *VertexBufferD3D12::GetHandle() { return m_VertexBuffer.Get(); }

IndexBufferD3D12::IndexBufferD3D12(GraphicsDeviceD3D12 *device,
                                   const BufferDescription &description,
                                   const void *data, IndexBufferFormat format)
...
void IndexBufferD3D12::SetData(const void *data, uint32_t size,
                               uint32_t offset) {
  void *buffer;
  m_UploadBuffer->Map(0, &m_UploadRange, &buffer);
  ...
  m_UploadBuffer->Unmap(0, &m_UploadRange);
  m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
    cmd->CopyBufferRegion(m_IndexBuffer.Get(), 0, m_UploadBuffer.Get(), 0,
                          m_Description.Size);
  });
}

UniformBufferD3D12::UniformBufferD3D12(GraphicsDeviceD3D12 *device,
                                       const BufferDescription &description,
                                       const void *data)
  ...
  d3d12Device->CreateCommittedResource(
      &bufferProperties, D3D12_HEAP_FLAG_NONE, &bufferDesc,
      D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&m_ConstantBuffer));
...
void UniformBufferD3D12::SetData(const void *data, uint32_t size,
                                 uint32_t offset) {
  void *buffer;
  m_UploadBuffer->Map(0, &m_UploadRange, &buffer);
  ...
  m_UploadBuffer->Unmap(0, &m_UploadRange);
  m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
    cmd->CopyBufferRegion(m_ConstantBuffer.Get(), 0, m_UploadBuffer.Get(), 0,
                          m_Description.Size);
  });
}
} // namespace Nexus::Graphics
