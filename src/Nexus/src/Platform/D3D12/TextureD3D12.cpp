...
{
    ID3D12Device10 *d3d12Device = device->GetDevice();
    m_TextureFormat = D3D12::GetD3D12PixelFormat(m_Specification.Format, false);
    ...
    // staging buffer
    {
        ...
        uploadBufferDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
        uploadBufferDesc.Alignment = D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT;
        uploadBufferDesc.Width = m_Specification.Width * m_Specification.Height * sizeInBytes;
        uploadBufferDesc.Height = 1;
        uploadBufferDesc.DepthOrArraySize = 1;
        uploadBufferDesc.MipLevels = 1;
        uploadBufferDesc.Format = DXGI_FORMAT_UNKNOWN;
    ...
    // set layouts
    {
        ...
        {
            std::vector<D3D12_RESOURCE_STATES> resourceStates;
            for (uint32_t mipLevel = 0; mipLevel < m_Specification.MipLevels; mipLevel++)
            {
                resourceStates.push_back(D3D12_RESOURCE_STATE_COMMON);
            }

            m_ResourceStates.push_back(resourceStates);
        }
    }
}
...
void Cubemap_D3D12::SetData(const void *data, CubemapFace face, uint32_t level, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
{
    ...
    UINT uploadNumRows;
    UINT64 uploadRowSizeInBytes;
    UINT64 uploadSizeInBytes;
    d3d12Device->GetCopyableFootprints1(&uploadBufferDesc, 0, 1, 0, &uploadFootprint, &uploadNumRows, &uploadRowSizeInBytes, &uploadSizeInBytes);

    D3D12_PLACED_SUBRESOURCE_FOOTPRINT cubemapFootprint;
    UINT cubemapNumRows;
    UINT64 cubemapRowSizeInBytes;
    UINT64 cubemapSizeInBytes;
    d3d12Device->GetCopyableFootprints1(&cubemapDesc, faceIndex, 1, 0, &cubemapFootprint, &cubemapNumRows, &cubemapRowSizeInBytes, &cubemapSizeInBytes);
    ...
    m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
        ...
        textureSizeAsBox.top = y;
        textureSizeAsBox.front = 0;
        textureSizeAsBox.right = x + width;
        textureSizeAsBox.bottom = y + height;
        textureSizeAsBox.back = 1;
        ...
        textureSource.pResource = m_UploadBuffer.Get();
        textureSource.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
        textureSource.PlacedFootprint = uploadFootprint;

        textureDestination.pResource = m_Texture.Get();
        textureDestination.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
        textureDestination.PlacedFootprint = cubemapFootprint;
