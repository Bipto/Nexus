...
Texture2D_D3D12::Texture2D_D3D12(GraphicsDeviceD3D12 *device,
                                 const Texture2DSpecification &spec)
    : Texture2D(spec, device), m_Specification(spec), m_Device(device) {
  bool isDepth;
  D3D12_RESOURCE_FLAGS flags =
      D3D12::GetD3D12ResourceFlags(spec.Usage, isDepth);
  ...
  // staging buffer
  {
    ...
    d3d12Device->CreateCommittedResource(
        &uploadProperties, D3D12_HEAP_FLAG_NONE, &uploadBufferDesc,
        initialState, nullptr, IID_PPV_ARGS(&m_UploadBuffer));
  }
  ...
  // texture
  {
    ...
    d3d12Device->CreateCommittedResource(&heapProperties, D3D12_HEAP_FLAG_NONE,
...
void Texture2D_D3D12::SetData(const void *data, uint32_t level, uint32_t x,
                              uint32_t y, uint32_t width, uint32_t height) {
  uint32_t stride = width * GetPixelFormatSizeInBytes(m_Specification.Format);
  uint32_t size =
      width * height * GetPixelFormatSizeInBytes(m_Specification.Format);
  ...
  m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
    ...
    cmd->CopyTextureRegion(&textureDestination, x, y, 0, &textureSource,
                           &textureBounds);
    cmd->ResourceBarrier(1, &toDefaultBarrier);
  });
}

std::vector<std::byte> Texture2D_D3D12::GetData(uint32_t level, uint32_t x,
  ...
  UINT64 totalBytes;
  d3d12Device->GetCopyableFootprints1(&textureDesc, level, 1, 0, &layout,
  ...
  d3d12Device->CreateCommittedResource(
      &readbackProperties, D3D12_HEAP_FLAG_NONE, &readbackBufferDesc,
...
DXGI_FORMAT Texture2D_D3D12::GetFormat() { return m_TextureFormat; }

const Microsoft::WRL::ComPtr<ID3D12Resource2> &
Texture2D_D3D12::GetD3D12ResourceHandle() {
  return m_Texture;
}

void Texture2D_D3D12::SetResourceState(uint32_t level,
...
Cubemap_D3D12::Cubemap_D3D12(const CubemapSpecification &spec,
                             GraphicsDeviceD3D12 *device)
    : Cubemap(spec, device), m_Device(device) {
  ID3D12Device10 *d3d12Device = device->GetDevice();
  ...
  // create resource states
  {
    for (uint32_t arrayLayer = 0; arrayLayer < 6; arrayLayer++) {
      ...
      for (uint32_t mipLevel = 0; mipLevel < m_Specification.MipLevels;
           mipLevel++) {
...
void Cubemap_D3D12::SetData(const void *data, CubemapFace face, uint32_t level,
                            uint32_t x, uint32_t y, uint32_t width,
                            uint32_t height) {
  ID3D12Device10 *d3d12Device = m_Device->GetDevice();
  uint32_t faceIndex = (uint32_t)face;
  uint32_t sizeInBytes = (width - x) * (height - y) *
                         GetPixelFormatSizeInBytes(m_Specification.Format);
  uint32_t subresource =
      Utils::CalculateSubresource(level, faceIndex, m_Specification.MipLevels);
  ...
  d3d12Device->GetCopyableFootprints1(&uploadBufferDesc, 0, 1, 0,
                                      &uploadFootprint, &uploadRows,
                                      &uploadBytesPerRow, &uploadTotalBytes);

  D3D12_PLACED_SUBRESOURCE_FOOTPRINT cubemapFootprint;
  UINT cubemapRows;
  UINT64 cubemapBytesPerRow;
  UINT64 cubemapTotalBytes;
  d3d12Device->GetCopyableFootprints1(&cubemapDesc, subresource, 1, 0,
                                      &cubemapFootprint, &cubemapRows,
                                      &cubemapBytesPerRow, &cubemapTotalBytes);
...
std::vector<std::byte> Cubemap_D3D12::GetData(CubemapFace face, uint32_t level,
  ...
  UINT64 rowSizeInBytes;
  UINT64 totalBytes;
  d3d12Device->GetCopyableFootprints1(&textureDesc, subresource, 1, 0, &layout,
                                      &numRows, &rowSizeInBytes, &totalBytes);
...
DXGI_FORMAT Cubemap_D3D12::GetFormat() { return m_TextureFormat; }

const Microsoft::WRL::ComPtr<ID3D12Resource2> &
Cubemap_D3D12::GetD3D12ResourceHandle() {
  return m_Texture;
}

void Cubemap_D3D12::SetResourceState(uint32_t arrayLayer, uint32_t mipLevel,
                                     D3D12_RESOURCE_STATES state) {
  m_ResourceStates[arrayLayer][mipLevel] = state;
}

D3D12_RESOURCE_STATES Cubemap_D3D12::GetResourceState(uint32_t arrayLayer,
                                                      uint32_t mipLevel) {
