...
SwapchainD3D12::SwapchainD3D12(Window *window, GraphicsDevice *device,
...
VSyncState SwapchainD3D12::GetVsyncState() { return m_VsyncState; }
...
SwapchainD3D12::RetrieveRenderTargetViewDescriptorHandle() const {
...
uint32_t SwapchainD3D12::GetColorAttachmentCount() { return 1; }
...
void SwapchainD3D12::ResizeBuffers() {
  ...
  m_Swapchain->ResizeBuffers(BUFFER_COUNT, m_SwapchainWidth, m_SwapchainHeight,
                             DXGI_FORMAT_UNKNOWN,
...
void SwapchainD3D12::GetBuffers() {
  ...
  for (size_t i = 0; i < BUFFER_COUNT; ++i) {
    ...
    d3d12Device->CreateRenderTargetView(m_Buffers[i].Get(), &rtv,
                                        m_RenderTargetViewDescriptorHandles[i]);
  }
}
...
void SwapchainD3D12::CreateColourAttachments() {
  ...
  HWND hwnd = (HWND)SDL_GetProperty(
      SDL_GetWindowProperties(m_Window->GetSDLWindowHandle()),
  ...
  factory->CreateSwapChainForHwnd(m_Device->GetCommandQueue(), hwnd,
  ...
  d3d12Device->CreateDescriptorHeap(
      &descriptorHeapDesc, IID_PPV_ARGS(&m_RenderTargetViewDescriptorHeap));

  // create handles to descriptor view
  auto firstHandle =
      m_RenderTargetViewDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
  auto handleIncrement = d3d12Device->GetDescriptorHandleIncrementSize(
...
void SwapchainD3D12::CreateDepthAttachment() {
  ...
  d3d12Device->CreateDescriptorHeap(
      &dsvHeapDesc, IID_PPV_ARGS(&m_DepthTextureDescriptorHeap));
  ...
  d3d12Device->CreateCommittedResource(
      &heapProperties, D3D12_HEAP_FLAG_NONE, &resourceDesc, m_CurrentDepthState,
      &depthOptimizedClearValue, IID_PPV_ARGS(&m_DepthBuffer));

  d3d12Device->CreateDepthStencilView(
      m_DepthBuffer.Get(), &depthStencilDesc,
...
void SwapchainD3D12::CreateMultisampledFramebuffer() {
  ...
  spec.ColorAttachmentSpecification = {
      Nexus::Graphics::PixelFormat::R8_G8_B8_A8_UNorm};
  spec.DepthAttachmentSpecification =
...
void SwapchainD3D12::Resolve() {
  if (m_MultisampledFramebuffer->GetFramebufferSpecification().Width >
      GetWindow()->GetWindowSize().X) {
    return;
  }

  if (m_MultisampledFramebuffer->GetFramebufferSpecification().Height >
      GetWindow()->GetWindowSize().Y) {
    return;
  }

  Ref<Texture2D_D3D12> framebufferTexture =
      std::dynamic_pointer_cast<Texture2D_D3D12>(
          m_MultisampledFramebuffer->GetColorTexture());

  DXGI_FORMAT format = D3D12::GetD3D12PixelFormat(
      Nexus::Graphics::PixelFormat::R8_G8_B8_A8_UNorm, false);
  D3D12_RESOURCE_STATES framebufferState =
      framebufferTexture->GetResourceState(0);
  ...
  m_Device->ImmediateSubmit([&](ID3D12GraphicsCommandList7 *cmd) {
    m_Device->ResourceBarrier(
        cmd, framebufferTexture->GetD3D12ResourceHandle().Get(), 0,
        framebufferState, D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
    m_Device->ResourceBarrier(cmd, swapchainTexture.Get(), 0, swapchainState,
                              D3D12_RESOURCE_STATE_RESOLVE_DEST);

    cmd->ResolveSubresource(swapchainTexture.Get(), 0,
                            framebufferTexture->GetD3D12ResourceHandle().Get(),
                            0, format);

    m_Device->ResourceBarrier(
        cmd, framebufferTexture->GetD3D12ResourceHandle().Get(), 0,
        D3D12_RESOURCE_STATE_RESOLVE_SOURCE, framebufferState);
    m_Device->ResourceBarrier(cmd, swapchainTexture.Get(), 0,
                              D3D12_RESOURCE_STATE_RESOLVE_DEST,
                              swapchainState);
  });
}
