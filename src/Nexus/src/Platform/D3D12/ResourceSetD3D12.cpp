...
ResourceSetD3D12::ResourceSetD3D12(const ResourceSetSpecification &spec,
  ...
  uint32_t textureConstantBufferCount =
      spec.SampledImages.size() + spec.UniformBuffers.size();
  ...
  if (textureCount > 0) {
    ...
    d3d12Device->CreateDescriptorHeap(&samplerDesc,
    ...
    for (int i = 0; i < textureCount; i++) {
      ...
      const uint32_t slot = ResourceSet::GetLinearDescriptorSlot(
          textureInfo.Set, textureInfo.Binding);

      m_SamplerCPUDescriptors[slot] = cpuLocation;
      m_SamplerGPUDescriptors[slot] = gpuLocation;

      cpuLocation.ptr += d3d12Device->GetDescriptorHandleIncrementSize(
          D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
      gpuLocation.ptr += d3d12Device->GetDescriptorHandleIncrementSize(
          D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
    }
  }
  ...
  if (textureConstantBufferCount > 0) {
    ...
    d3d12Device->CreateDescriptorHeap(
        ...
        IID_PPV_ARGS(&m_TextureConstantBufferDescriptorHeap));

    auto cpuLocation = m_TextureConstantBufferDescriptorHeap
                           ->GetCPUDescriptorHandleForHeapStart();
    auto gpuLocation = m_TextureConstantBufferDescriptorHeap
    ...
    // create constant buffer handles
    {
      for (int i = 0; i < spec.UniformBuffers.size(); i++) {
        ...
        const uint32_t slot = ResourceSet::GetLinearDescriptorSlot(
            constantBufferInfo.Set, constantBufferInfo.Binding);
...
void ResourceSetD3D12::WriteUniformBuffer(Ref<UniformBuffer> uniformBuffer,
  ...
  auto d3d12Device = m_Device->GetDevice();
  Ref<UniformBufferD3D12> d3d12UniformBuffer =
      std::dynamic_pointer_cast<UniformBufferD3D12>(uniformBuffer);
  ...
  d3d12Device->CreateConstantBufferView(
      &desc, m_ConstantBufferCPUDescriptors.at(index));

  m_BoundUniformBuffers[name] = uniformBuffer;
}

void ResourceSetD3D12::WriteCombinedImageSampler(Ref<Texture2D> texture,
                                                 Ref<Sampler> sampler,
                                                 const std::string &name) {
  const auto d3d12Device = m_Device->GetDevice();
  // write texture
  {
    Ref<Texture2D_D3D12> d3d12Texture =
        std::dynamic_pointer_cast<Texture2D_D3D12>(texture);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE textureHandle =
        m_TextureCPUDescriptors.at(index);
    auto resourceHandle = d3d12Texture->GetD3D12ResourceHandle();
    d3d12Device->CreateShaderResourceView(resourceHandle.Get(), &srv,
                                          textureHandle);
  }
  ...
  // write sampler
  {
    ...
    const uint32_t index = GetLinearDescriptorSlot(info.Set, info.Binding);
    auto d3d12Device = m_Device->GetDevice();
    Ref<SamplerD3D12> d3d12Sampler =
        std::dynamic_pointer_cast<SamplerD3D12>(sampler);

    const auto &spec = d3d12Sampler->GetSamplerSpecification();

    const glm::vec4 color =
        Nexus::Utils::ColorFromBorderColor(spec.TextureBorderColor);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE samplerHandle =
        m_SamplerCPUDescriptors.at(index);
...
void ResourceSetD3D12::WriteCombinedImageSampler(Ref<Cubemap> cubemap,
                                                 Ref<Sampler> sampler,
                                                 const std::string &name) {
  const auto d3d12Device = m_Device->GetDevice();
  ...
  // write cubemap
  {
    Ref<Cubemap_D3D12> d3d12Cubemap =
        std::dynamic_pointer_cast<Cubemap_D3D12>(cubemap);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE cubemapHandle =
        m_TextureCPUDescriptors.at(index);
...
ResourceSetD3D12::GetTextureConstantBufferDescriptorHeap() {
...
ResourceSetD3D12::GetSamplerStartHandle() const {
  return m_SamplerDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
}

const D3D12_GPU_DESCRIPTOR_HANDLE
ResourceSetD3D12::GetTextureConstantBufferStartHandle() const {
  return m_TextureConstantBufferDescriptorHeap
      ->GetGPUDescriptorHandleForHeapStart();
}
} // namespace Nexus::Graphics
