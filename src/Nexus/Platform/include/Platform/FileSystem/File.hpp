...
namespace Nexus::IO
{
    ...
    enum class FileMode
    {
        ...
        /// @brief File opened for writing only, creates a new file or overwrites an
        /// existing file
        WriteText,

        /// @brief File opened for append only, creates a new file if it doesn't
        /// exist. New content is added to the end of the file
        AppendText,

        /// @brief File opened for both reading and writing, must already exist
        ReadWriteText,

        /// @brief Create a new file for reading and writing, overwriting any
        /// existing file
        WriteNewText,

        /// @brief File is opened for reading and appending, creates a new file if it
        /// doesn't exist
        ReadAppendText,

        /// @brief File opened for reading only, must already exist. Reading is done
        /// in binary mode
        ReadBinary,

        /// @brief File opened for writing only, creates a new file or overwrites an
        /// existing file. Writing is done in binary mode
        WriteBinary,

        /// @brief File opened for append only, creates a new file if it doesn't
        /// exist. New content is added to the end of the file. Appending is done in
        /// binary mode
        AppendBinary,

        /// @brief File opened for both reading and writing, must already exist.
        /// Reading and writing is done in binary mode
        ReadWriteBinary,

        /// @brief Create a new file for reading and writing, overwriting any
        /// existing file. Reading and writing is done in binary mode
        WriteNewBinary,

        /// @brief File is opened for reading and appending, creates a new file if it
        /// doesn't exist. Reading and appending is done in binary mode
        ReadAppendBinary
    };

    /// @brief An enumeration representing different origins for seeking within a
    /// file
