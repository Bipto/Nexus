...
namespace Nexus
{
    ...
    class EventHandler
    {
      ...
      public:
        ...
        {
            for (const auto &[id, delegate] : m_EventFunctions)
            {
                delegate(param...);
            }
        }
        ...
        {
            /// @note: Using current time as ID may lead to collisions if multiple
            /// events are bound in the same time frame. Consider using a more robust
            /// key, possibly a GUID or a static incrementing counter.

            static std::atomic<uint64_t> s_EventIDGenerator = 0;

            EventID id = s_EventIDGenerator++;
