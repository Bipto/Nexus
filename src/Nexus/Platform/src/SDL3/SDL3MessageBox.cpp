...
namespace Nexus
{
    static Uint32 GetMessageBoxButtonFlags(DefaultKey key)
    {
        switch (key)
        {
        case DefaultKey::Escape:
            return SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT;
        case DefaultKey::Return:
            return SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT;
        case DefaultKey::NoKey:
            return 0;
        default:
            throw std::runtime_error("Failed to find a valid default key");
        }
    }
    ...
    static Uint32 GetMessageBoxFlags(const MessageBoxDescription &desc)
    {
        ...
        default:
            throw std::runtime_error("Failed to find a valid message box type");
        }
        ...
        default:
            throw std::runtime_error("Failed to find a valid layout");
        }
