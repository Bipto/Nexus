...
namespace Nexus
{
    class SDL3Window final : public IWindow
    {
      public:
        ...
        /// the window
        std::pair<uint32_t, uint32_t> GetWindowSize() final;
        ...
        std::pair<float, float> GetMousePosition(uint32_t mouseId) final;
        std::pair<float, float> GetMouseScroll(uint32_t mouseId) final;
        std::optional<std::pair<float, float>> GetMousePosition() final;
        std::optional<std::pair<float, float>> GetMouseScroll() final;
