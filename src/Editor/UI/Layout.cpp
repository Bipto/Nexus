...
namespace Editor
{
	...
	Layout::Layout(Nexus::Application *app) : m_Application(app)
	{
	...
	void Layout::RenderViewport()
	{
		...
		ImGuiWindowFlags flags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
		...
		if (ImGui::Begin("Dockspace", nullptr, flags))
		{
			...
			for (const auto &panel : m_Panels) { panel.second->OnRender(); }

			for (const auto &dialog : m_Dialogs) { dialog.second->OnRender(); }
		}
		ImGui::End();
	}
