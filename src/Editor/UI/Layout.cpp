...
Layout::Layout(Nexus::Application *app) : m_Application(app) {
  m_ImGuiRenderer =
      std::make_unique<Nexus::ImGuiUtils::ImGuiGraphicsRenderer>(app);
  ...
  m_Panels[SCENE_VIEWPORT_NAME] = std::make_unique<SceneViewport>(
      app->GetGraphicsDevice(), m_ImGuiRenderer.get());
