...
SceneViewport::SceneViewport(
    Nexus::Graphics::GraphicsDevice *graphicsDevice,
    Nexus::ImGuiUtils::ImGuiGraphicsRenderer *imGuiGraphicsRenderer)
    : m_GraphicsDevice(graphicsDevice),
      m_ImGuiGraphicsRenderer(imGuiGraphicsRenderer) {
...
void SceneViewport::OnRender() {
  if (m_Enabled) {
    ...
    if (ImGui::Begin(SCENE_VIEWPORT_NAME, &m_Enabled)) {
      ...
      if (size.x != m_CurrentWidth || size.y != m_CurrentHeight ||
...
void SceneViewport::ResizeViewport(uint32_t width, uint32_t height) {
  ...
  spec.DepthAttachmentSpecification.DepthFormat =
  ...
  m_ViewportTextureID = m_ImGuiGraphicsRenderer->BindTexture(
      m_ViewportFramebuffer->GetColorTexture(0));
}
...
void SceneViewport::RenderScene() {
  ...
  m_CommandList->SetRenderTarget(
      Nexus::Graphics::RenderTarget{m_ViewportFramebuffer});
  m_CommandList->ClearColorTarget(
      0, {clearColour.r, clearColour.g, clearColour.b, clearColour.a});
  m_CommandList->End();
